...
| Claim | The author claims the ticket so concurrent agents back off. See the claim protocol below. | `bug-fix` / `feature-implementation` workflow, or a human | Claim visible on the ticket |
...
| Post-merge housekeeping | Close issues the merged PR fixes; comment on issues whose PRs were closed without merging; turn leftover asks or review comments into tracked follow-up issues. | `xezar-integration` close-out (`.xezar/docs/close-out.md`) | Tracker reconciled, follow-ups filed |
