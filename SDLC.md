...
| Independent review | Whether the solution is sound, read-only, by someone other than the author. Architecture is a **named section of this review**: the reviewer checks the boundary changes against the plan, and scales that section by risk. Where the leader wants a separate reading of the design, the read-only `architecture-review` workflow (`xezar-architecture-review`) posts it as a `## Architecture review` comment; it informs the code review and never replaces it or moves its labels. | The review verdict at a named head, with each finding's disposition ([CODE_REVIEW.md](CODE_REVIEW.md)) |
...
A review, QA or design verdict is posted where it always is – the PR comment above – and the reviewing step also leaves a machine-readable packet that the engine records on its task. There are four verdict roles, each with its own words, never translated: **`code-review`** (APPROVE or REQUEST CHANGES), **`design-review`** (PASS, PASS WITH FOLLOW-UPS or FAIL), **`qa`** (PASS or FAIL) and **`architecture-review`** (APPROVE or REQUEST CHANGES). The engine records a packet only when the reviewing step declares that role with `verdictRole` in its workflow; a packet from any other step is refused and noted on the task (#851). A kit verdict workflow therefore declares the role on its verdict step – `code-review`, `design-review`, `qa` and `architecture-review` do – and a recorded verdict is evidence of what the reviewer said, never a label and never approval on its own.
...
Dependency installation runs alone. The canonical kit runner may then overlap three lanes: `typecheck → build → test:package`, `npm test`, and `npm run test:unit`. Join all lanes before `.xezar/checks/repository-checks.sh` (actual catalog, changelog, link and contract checks). Run `bash .xezar/checks/infra-tests.sh` locally as well for kit-check/workflow changes; its unconditional `Xezar infrastructure fixtures` CI job is required on every PR. Serial execution remains valid. Preserve the Vitest worker cap, execute every required command even after an ordinary gate failure, and leave cancelled command phases or unrecordable attempts incomplete. Atomic result publication is the completion commit point: if it finishes before a deferred cancellation is handled, retain and report the completed verdict. Only one reducer writes aggregate gate evidence. This schedule changes no command or acceptance requirement.
