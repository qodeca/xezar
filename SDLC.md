...
A review, QA or design verdict is posted where it always is – the PR comment above – and the reviewing step also leaves a machine-readable packet that the engine records on its task. There are four verdict roles, each with its own words, never translated: **`code-review`** (APPROVE or REQUEST CHANGES), **`design-review`** (PASS, PASS WITH FOLLOW-UPS or FAIL), **`qa`** (PASS or FAIL) and **`architecture-review`** (APPROVE or REQUEST CHANGES). The engine records a packet only when the reviewing step declares that role with `verdictRole` in its workflow; a packet from any other step is refused and noted on the task (#851). A kit verdict workflow therefore declares the role on its verdict step – `code-review`, `design-review` and `qa` do – and a recorded verdict is evidence of what the reviewer said, never a label and never approval on its own.
