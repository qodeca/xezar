...
- No new **server runtime** dependencies without strong justification — that dependency budget is hono, @hono/node-server, yaml, zod, smol-toml, ws, @clack/prompts and picocolors, and nothing else. The list is exhaustive on purpose: adding to it is a review decision, so the commit that widens it updates this line and says why. (`ws` earned its place because Node ships a WebSocket *client* but no server and `@hono/node-server` provides none, so the `/api/v1/ws` subscription bus had no in-tree option. `@clack/prompts` is the `server-install` wizard's terminal UI, and the whole module — dependency included — is lazy-imported from `packages/xezar/src/index.ts`, so an ordinary `serve` never loads it.) `picocolors` supplies terminal colour formatting with no runtime dependencies, approved in #467 Q1; layout and sanitization stay local. Browser packages are build-time dependencies and must remain locked, bundle-measured, and absent from the installed CLI's runtime dependency graph.
