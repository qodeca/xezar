...
| **OpenCode** _(experimental)_ | `opencode serve`, HTTP + SSE | Ignores `allowedTools`; a permission ask for a path outside the run's own directories is allowed if it resolves inside them, denied otherwise |
