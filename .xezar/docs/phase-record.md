...
- `DELIVERED` authorizes and records one review-response destination. Three lines: `branch`, `head`, `base`. Before delivery, `worktree-git.sh push <branch>` accepts only that branch, requires this task at `head`, and requires the live remote at `base`; it constructs a normal non-force push itself. Afterwards readiness and both evidence modes validate the complete record and live tip even when this task has commits, and require the current response/seal at `head`. A missing field, concurrent advancement, stale live tip or superseded-round head refuses by a named predicate. The older empty-branch compatibility shape remains readable, but a no-op seal at the task base cannot certify the response.
- `VERIFICATION` is the verify-only case only — this run verified an existing revision and was never asked to change source. It is `verified:` plus `findings:`, and it is why readiness accepts an empty branch. It pushes nothing; handoff reports the verified revision and says explicitly that this task's seal covers only its unchanged checkout, not the verified PR bytes. It is **not** the acceptance-criteria mapping; that is `AC_VERIFICATION`, above, and the two are different questions.
...
Two compatibility run shapes legitimately carry no commits of their own — `DELIVERED` (an older review-response path put the fix only on the PR's own branch) and `VERIFICATION` (this run only verified an existing revision) — and readiness accepts an empty branch for exactly those two. For them the empty change set is expected. `security-scan.sh` records the declaration with its reason so a reviewer reads why the branch is empty; an empty change set resolves as `not-applicable` whether or not it is declared, and the declaration is never a pass. `DELIVERED` still cannot hand off a seal for that empty tree when its pushed head differs.
