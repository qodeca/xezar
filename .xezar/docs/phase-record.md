...
Four statuses, and only the first is a pass: `pass`, `findings` (the gate fails), `unknown`, `not-applicable`. A candidate whose base was resolved, whose diff was readable, and whose inventory is nonetheless **empty** is refused rather than passed — it did not look. The refusal is driven off the inventory, not off the decision, because the decision for a zero-file inventory is always "no" and a refusal keyed to "yes" could never fire.

Two run shapes legitimately carry no commits of their own — `DELIVERED` (the fix went to the PR's own branch) and `VERIFICATION` (this run only verified an existing revision) — and readiness accepts an empty branch for exactly those two. For them the empty change set is expected, so `security-scan.sh` **declares** it, with the reason, and the declaration is recorded in the result next to a status that is still `unknown`. The declaration is the only escape, it is never inferred from the diff, and it is never a pass.
