...
- `DELIVERED` is the compatibility path for a review-response fix already pushed to the PR's own branch after this task's branch was left empty. Three lines: `branch`, `head`, `base`, checked live against the remote. New review-response work retains the SAME response commit on this task's branch and pushes that exact commit to the PR branch, so the workflow gates and seals the pushed head. If an older empty-branch path carries `DELIVERED`, handoff additionally refuses unless the seal head equals its pushed `head`; a no-op seal at the task base cannot certify the response.
