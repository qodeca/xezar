...
| Reduce conflicts in the shared task ledger | The #289 handoff collision in `dogfooding.md` required a refresh and full revalidation. | Owner decision D3 on #447: keep one file, sorted newest-first. A split/archive is deferred until another collision justifies it. |
