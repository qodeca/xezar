...
- **Never allowed:**
  ...
  4. The kit's read-only roles (code-review, design-review, qa, architecture-review, business-analysis) routed to OpenCode – it applies no tool limit at all (AGENTS.md: OpenCode NOT APPLIED), so a "read-only" step there is not read-only. OpenCode also stays out of the rotation for writing rows until a repeated writing retrial reaches a draft PR (§ 7). Ornith (dropped).
...
- The kit's five read-only roles (code-review, design-review, qa, architecture-review, business-analysis) are never routed to OpenCode: it applies no tool limit at all (§ 6's per-runner row; AGENTS.md: OpenCode NOT APPLIED), so a step marked read-only is not read-only on that runner, whatever else changes – this holds regardless of #692. Out of the rotation for writing rows: the OpenCode runner, until a repeated writing retrial reaches a draft PR. The 2026-09-19 qualification (§ 13) ran 11 read-only tasks of five workflow types to `done` on three models, and the whole writing chain on build `9e97a149` up to a sealed evidence set – but a rejected `external_directory` ask can leave a session silent for over ten minutes (**#692**, four reproductions), which stalled the one writing retrial at `handoff` before it could open a PR. #692 is closed (2026-09-19); the remaining condition for writing-row re-entry is a repeated retrial reaching a draft PR. Before that trial the record was eight stalls to 2026-09-16 and the leader's 2026-09-18 tally of 9 of 9 (§ 13). Also out: Ornith (worked in the primary checkout), `gpt-5.5` (retires 2026-10-14), haiku for reviews, `gpt-5.3-codex-spark` for fixes. Qwen on pi: allowed for one-shot tracker work only; it can loop after it finishes.
...
| OpenCode runner | 2026-09-19 qualification: 11 read-only runs of five workflow types, all `done`, on three models (1m53s–9m22s), and the whole writing chain ran to a sealed evidence set on build `9e97a149` – then stalled at `handoff` on the #692 post-denial silence, so no PR was opened. Still out for the kit's five read-only roles (§ 7, permanent); writing re-entry needs a repeated retrial reaching a draft PR (see § 7 and § 13); the pre-trial record was eight stalls to 2026-09-16 plus the leader's 2026-09-18 tally of 9 of 9 |
...
| `dgx-spark/deepseek-v4-flash-vision` | opencode | same model | – | **Stalled 7/7** across two days to 2026-09-18 (`f07d1839`, `5ae9de7e`, `43c73d0f`, `cd20e3a8`, `ae6280b2`, `3e608a1d`, #373 docs); the 2026-09-19 re-trial on the #578-fixed runner completed a read-only business-analysis (`f4bec267`, #678, 5m59s) – the runner is still out for the kit's five read-only roles (§ 7); writing re-entry needs a repeated retrial reaching a draft PR – D:165–168, R; see § 13 |
...
| `mac-m4/qwen38…` | opencode | same model | – | Stalled 1/1 (`52a11f58`) to 2026-09-18; the 2026-09-19 re-trial completed a read-only business-analysis re-check (`4f40f4b7`, #686, 4m06s) – the runner is still out for the kit's five read-only roles (§ 7); writing re-entry needs a repeated retrial reaching a draft PR – R; the 2026-09-16 default-model pin and kit-snapshot outcome are traceable in § 13 |
...
## 12. Sources
The dated source inventory is maintained with the evidence in [model-routing-findings.md](model-routing-findings.md).
## 13. Findings log
The findings log and expired 0.16.0 history are maintained in [model-routing-findings.md](model-routing-findings.md).
## Glossary
