...
| `opus` / `opus[1m]` (`claude-opus-5`) | claude | $5 / $25; "Moderate"; several times more quota per turn than Sonnet | "Start with Opus 5 for most workloads": multi-hour coding agents, large refactors, computer use | Worked: UI implementation, deep re-reviews with live probes (`0944c453` found 2 blockers), conflict repairs – D:105, D:113, R; leader-reported: PR #474 review and #466 P4 implementation were good work (leader log, runs `2305b394`, `cedba1c1`) |
...
| `gpt-6-astra` | codex | Top; $10 / $50; 1.05M ctx (272k in Codex); effort low→ultra; shorter output per task | "Most capable model, built for the hardest end-to-end work"; "when a task needs the strongest capability across multiple steps and tools" | Best reviewer of 2026-09-13 (8 real majors on #403/#404), spike `dfc69665` clean; merged #502 in 14 min – D:115, D:316, R; leader-reported: correct docs sweeps in areas D and B (leader log, runs `a1dd214f`, `315c563e`) |
| `gpt-5.6-sol` (alias `gpt-5.6`) | codex | Flagship; $4 / $20; effort none→max | "Ambiguous, difficult, or high-value tasks that need extra analysis, judgment, or polish" | Worked: review `363b3e29` with a solid major, found a case-fold routing bug, full writing step once the brief was fixed – D:106, D:256; leader-reported: thorough issue #515 audit (leader log, run `bfa65dc8`). Response `d2ec4451`, re-check `47c7f4bf`, and investigation `deb5deb4` were in progress at the log time, without a verdict |
| `gpt-5.6-terra` | codex | Mid (the old "mini" slot); $2 / $12 | "Everyday work that needs strong reasoning and tool use when you do not need Sol's full depth" | Mixed: implementation `4571a595` thin; QA reliable and honest; bounded fix merged as #412 – D:110, D:255, D:318. Leader-reported: reliable for procedural work, including PR, review, and honest CI-stop outcomes, but literal about conditional integration briefs (leader log, runs `77df8015`, `baa0a90b`, `325bcea0`, `d635efc4`, `8b56fd32`) |
...
| `mac-m4/qwen38…` | opencode | same model | – | Stalled 1/1 (`52a11f58`) – R; the 2026-09-16 default-model pin disrupted kit snapshot checks when set from the cockpit (leader log, runs `f0a3e70a`, `8bc1287b`, `9303ee84`) |

Quota facts that drive the map: a Claude Code Pro/Max subscription has a 5-hour session window plus weekly caps, shared across all Claude models, so switching from Opus to Sonnet inside a spent window does not help ("You've hit your session limit · resets 11:50am" = the window is spent; it hit at 06:34 and 09:05 on 2026-09-16 and stopped 7 runs at once). Leader-reported: a separate limit at 20:12Z on 2026-09-15 failed four runs (`cedba1c1`, `0dd5552a`, `4c4428fa`, `b8d86881`); switching accounts then continuing retained context (leader log, runs `cedba1c1`, `0dd5552a`, `4c4428fa`, `b8d86881`). Codex quota is separate per tier. The local models cost nothing but time (3–5× slower) and one worker slot each.
...
7. **Terra integration briefs use explicit ALLOWED actions, never conditions.** For example, say to run `gh api -X PATCH … -f base=main` regardless of `mergeStateStatus`; never permit `gh pr update-branch` in an exact-head brief. Conditional wording led to an integration skip on #511 and an exact-head refusal after `update-branch` moved #510's head (leader log, runs `d635efc4`, `8b56fd32`).
...
9. **Do not change a runner default model from the cockpit while the kit is in use.** Set it per task, or land the committed pin through a PR first. The cockpit write to the primary `.xezar/config.json` made kit bootstrap reject its snapshot; `quick-task` with a `xezar-*` skill avoided that kit step (leader log, runs `f0a3e70a`, `8bc1287b`, `9303ee84`).
...
- Leader log: leader-recorded observations from 2026-09-15 evening and 2026-09-16, cited per run in §5; leader-reported where this update could not find the task evidence.
...
- **OpenCode pin and kit snapshots — leader-reported.** At about 12:40, setting `defaultModels.opencode = mac-m4/qwen38-flash-next-mlx-mixed-4-8bit` from the cockpit wrote the primary checkout's `.xezar/config.json`. Kit bootstrap then refused chains 29 and 30 with `existing task asset differs: config.json` at `kit` (leader log, runs `f0a3e70a`, `8bc1287b`). `quick-task` with a `xezar-*` skill worked because it has no kit step. The owner chose to commit the pin in PR #513; after CI found the repository's own fixture asserting that the key was absent, the choice was to keep the pin and fix the test (leader log, run `9303ee84`).
- **Codex Terra — leader-reported.** `77df8015` opened PR #513 in about five minutes with both kit checks; `baa0a90b` reviewed it with real checks, APPROVE, and labels; `325bcea0` honestly stopped integration on a real `settings-agents.e2e.ts` failure rather than the #446 flake. Chains 20 and 21 merged #500 and #504 in about 14 minutes (leader log, runs `77df8015`, `baa0a90b`, `325bcea0`, `7cb254c2`, `f14cb943`).
- **Codex Sol — leader-reported.** The issue #515 audit was thorough, file:line-cited, and separated verified from unverified claims in about seven minutes (leader log, run `bfa65dc8`). At log time, the token-heavy response (about 12M tokens), re-check, and bug investigation had no verdict yet (leader log, runs `d2ec4451`, `47c7f4bf`, `deb5deb4`).
- **Codex Astra — leader-reported.** Area D's docs sweep covered 34 files in 16 minutes and 5.5M tokens, completed with kit checks; area B was also correct (leader log, runs `a1dd214f`, `315c563e`).
- **Existing local-model trials.** The already-recorded OpenCode results total 8/8 stalls across the two local models (`f07d1839`, `5ae9de7e`, `43c73d0f`, `cd20e3a8`, `ae6280b2`, `3e608a1d`, and `52a11f58`); pi Qwen's already-recorded loop occurred after posting (run `e12a2004`). No new trial verdict is added here.

### 2026-09-15 evening

- **Codex Terra — leader-reported.** In chain 27, #511 was skipped because its brief permitted a base refresh only when GitHub said BEHIND; GitHub said CLEAN although `main` had advanced, and the run posted `## Integration SKIPPED` (leader log, run `d635efc4`). In chain 28, a permitted `gh pr update-branch` moved #510's head from `3384a5d` to `1b8dbe8`, after which the exact-head guard correctly refused to proceed (leader log, run `8b56fd32`). The lesson is procedural reliability with literal brief interpretation, captured in rule 7.
- **Claude Opus — leader-reported.** The #474 review re-measured MCP coverage and re-ran two red proofs in eight minutes at $6.54; #466 P4 implementation changed 26 files and named 10 breaks at $15.50 (leader log, runs `2305b394`, `cedba1c1`). When the Claude session limit hit at 20:12Z, it failed `cedba1c1`, `0dd5552a`, `4c4428fa`, and `b8d86881`; after an account switch, `execution_control continue` retained context for each. Reviews moved to Codex during the outage. One continued chain resumed only its agent step and ended `done` without push, PR, or gates; a manual second continue was needed (leader log, run `cedba1c1`).
- **pi DeepSeek — leader-reported.** The tracker-only issue #478 filing was correct in one shot (leader log, run `a3f24c69`).
