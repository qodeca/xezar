...
| Every model – dispatching `code-review`, `design-review`, `qa` or `business-analysis` to pi (from 2026-09-22) | the four kit read-only workflows carry a `bashAllowlist` since #849 (kit slice D) – plain `Bash` in `allowedTools` was unrestricted on every backend, so a read-only role could write through the shell even though `Edit`/`Write` were already absent. On Claude the step now gets `Bash(<prefix>:*)` entries only (unchanged read-only capability). On pi the shell is DROPPED entirely: pi has no command-prefix mechanism, so `piTools` removes the whole `bash` tool once a `bashAllowlist` is set – a pi read-only task on one of these four roles cannot run `gh`/`git`/anything else in a shell. Dispatch these four to Claude or Codex until pi grows a narrower mechanism; the table's rows above are otherwise unchanged (§ 4 lane placements move only once #849's engine slice C also lands, per the issue's ordering rule) |
