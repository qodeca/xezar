...
- **Never allowed:**
  ...
  4. The kit's read-only roles (code-review, design-review, qa, architecture-review, business-analysis) routed to OpenCode – it applies no tool limit at all (AGENTS.md: OpenCode NOT APPLIED), so a "read-only" step there is not read-only. Any OpenCode re-entry is for other rows only (§ 7). Ornith (dropped).
...
- The kit's five read-only roles (code-review, design-review, qa, architecture-review, business-analysis) are never routed to OpenCode: it applies no tool limit at all (§ 6's per-runner row; AGENTS.md: OpenCode NOT APPLIED), so a step marked read-only is not read-only on that runner, whatever else changes. The 2026-09-19 qualification (§ 13) ran 11 read-only tasks of five workflow types to `done` on three models, and the whole writing chain on build `9e97a149` up to a sealed evidence set – but a rejected `external_directory` ask can leave a session silent for over ten minutes (**#692**, four reproductions), which stalled the one writing retrial at `handoff` before it could open a PR. #692 is closed (2026-09-19). Any OpenCode re-entry is for other rows only – never the five read-only roles above. Before that trial the record was eight stalls to 2026-09-16 and the leader's 2026-09-18 tally of 9 of 9 (§ 13). Also out: Ornith (worked in the primary checkout), `gpt-5.5` (retires 2026-10-14), haiku for reviews, `gpt-5.3-codex-spark` for fixes. Qwen on pi: allowed for one-shot tracker work only; it can loop after it finishes.
