...
6. Issues stay open. Never put a closing verb next to an issue number, not even to negate it: GitHub's
   ...
   #670 was closed on a `ci-watch` "passed" event and reopened the same minute when main CI was read
   as red; #677 was closed by a merge's closing keyword with B3–B6 still open (leader, 2026-09-20).
...
- Every Major or Blocker claim from a weaker model is verified by Opus before it reaches the owner. A
  ...
  such a claim is open the PR is HELD — `merge-queue` is removed — until a strong model that is
  neither the author nor the claimant re-proves it on a throwaway: on 2026-09-21 a DeepSeek advisory
  claimed a Blocker on PR #791 that the sonnet APPROVE had not tested, the hold was lifted only by
  Fable's independent verification (run `866c92bd`), the author being opus and the claimant DeepSeek
  (leader, 2026-09-21).
...
- Probe recipe: one tiny `quick-task` per account, all in one message so they run in parallel, with
  ...
  2026-09-21 that probe answered "Your workspace is out of credits. Add credits to continue." — a
  credits problem, not a window limit — and routing moved to state 2 (leader, 2026-09-21;
  `.xezar/docs/model-routing.md` § 5).
...
- Machine hygiene: pull the primary after every merge; at most two quality-gate runs at once; no new
  ...
  holds, because resuming three parked runs plus two fresh fixes put four gates in flight, drove the
  load to 39–47 and into the 90 % failure zone, and two runs then exhausted their gate-return
  counters on one flake (leader, 2026-09-21).
...
- Run gates in the foreground and wait. Never end a turn while a long job runs in the background: the
  ...
  non-final step is polled in one turn: Codex cannot host the wait, a Claude step must not end on
  `XEZ:MONITORING` or a `ScheduleWakeup`, and a second vitest in the same `TMPDIR` wipes the SSR
  cache — so a poll that re-runs the suite is not a substitute for waiting (leader, 2026-09-21; two
  attempts of #734's round 1 failed on exactly that).
