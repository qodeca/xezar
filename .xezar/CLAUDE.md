...
- `workflows/`: 13 role workflows. Eight development workflows retain preflight → setup → author → readiness → gates → evidence → handoff, with at most two gate-repair returns. Integration and root synchronization are separate roles. The final step stays an agent for interactive questions.
- `skills/`: 14 `xezar-*` roles: triage, business analysis, planning, implementation, bug investigation, code review, review response, testing, docs, dependencies, release preparation, quality gates, handoff and integration. The selected backend/model is inherited rather than pinned to another vendor.
