...
**Never kill a process by command-line pattern.** `pkill -f <pattern>`, `killall` and `kill $(pgrep -f …)` match every process this user owns anywhere on the machine, not just this task's. Xezar starts each agent CLI with its entire skill text as one `--append-system-prompt` argument, so any pattern that appears in a skill — `repo-gates.sh --fast` is the proven one, it is in all 16 — is present in the command line of every peer agent running that skill. `pkill` then SIGTERMs all of them and spares only the caller and its ancestors, so the task that fired it never sees what it did: that is #156, five agents killed mid-review across two events. Kill your own children with `pkill -P $$`, or save the PID when you start the process and kill that PID. If a pattern is genuinely unavoidable, anchor it to this task's own worktree path and inspect the match list first with `pgrep -fl <pattern>`, which matches identically and signals nothing. A run whose CLI dies on a signal xezar did not send now says so in its error message rather than reporting a bare exit code.
