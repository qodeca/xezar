...
The canonical gate preserves the repository's required order: `npm run typecheck`, `npm test`, `npm run test:unit`, `npm run build`, `npm run test:package`; dependency preparation and kit fixtures surround those five commands. UI smoke/manual QA is separate per SDLC and is not a pass when unavailable. Its maintained browser guide is the repository's `docs/testing/agent-browser.md` (repo root, not `.xezar/docs/`). Never replace real gates with synthetic fixture results or use `npx` to fetch a different test runner.
