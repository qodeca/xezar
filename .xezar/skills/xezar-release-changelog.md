...
| Signal | Heading |
|---|---|
| label `breaking`, or `!` before the `:` in the title | `## 💥 Breaking` |
| label `security` | `## 🔒 Security` |
| label `bug` | `## 🐛 Fixes` |
| label `feature` or `enhancement` | `## ✨ Features` |
| label `refactor` | `## 🔧 Changed` |
| label `documentation` | `## 📝 Specs & Documentation` |
| no category label, and the diff only touches `.github/`, `scripts/`, `.xezar/`, `.ai/`, CI config or test files | `## 🚀 CI/CD & Infrastructure` |
