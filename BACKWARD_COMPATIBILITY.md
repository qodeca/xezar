...
- **Commands:** bare invocation = `serve` (cockpit); `xezar run "<task>"`; `xezar init`; `xezar mcp` (the stdio MCP bridge a coding agent starts); `xezar lease gates -- <command>` (#672 — 0.17.0); `xezar server-install` / `server-deploy` / `server-uninstall` (the hosted-instance provisioner).
