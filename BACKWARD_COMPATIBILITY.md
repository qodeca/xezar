...
- **Commands:** bare invocation = `serve` (cockpit); `xezar run "<task>"`; `xezar init`; `xezar mcp` (the stdio MCP bridge a coding agent starts); `xezar lease gates -- <command>` (#672 — 0.17.0); `xezar lease gates --probe` (#838 — 0.19.0); `xezar accounts import-global` (#819 — 0.18.0); `xezar providers connect <provider> [--account <id>]` (#819 item 8 — 0.18.0); `xezar state-names [--json]` (#852 — 0.19.0); `xezar server-install` / `server-deploy` / `server-uninstall` (the hosted-instance provisioner).
...
- **`xezar lease gates --probe`** (#838 item B — 0.19.0) is the capability check a caller runs instead of reading the usage text. Its contract: exit 0; standard output is exactly one JSON object on one line — `{"lease":{"gates":true},"slots":<n>}`, where `slots` is `resources.gateSlots` as that launch resolves it (the project's own value in single-project mode, 1 when unset) — and nothing else; it runs nothing, takes no slot and writes no file in any layout; and its answer does not depend on whether the slot directory exists or can be written, because it reports capability, not health. The shape is pinned by `leaseProbeSchema` (`packages/xezar/src/lease-probe.ts`) and its exact bytes by `lease-probe.test.ts`; it lives beside the command rather than in `packages/contract`, which is the HTTP contract. Removing or renaming a key, changing a value's type, adding a line or anything else to stdout, or a non-zero exit where the lease exists is breaking; adding a key is additive, so a reader must ignore keys it does not know. `--probe` counts only before `--` — after it the flag belongs to the wrapped command, as it always did. **The `usage: xezar lease gates …` text on stderr is NOT a contract and may change in any release**, as may the wording of every other human-readable line the verb prints; a caller that needs to know whether the lease exists runs the probe. (The text is unchanged in 0.19.0; only its status is.)
...
- **`xezar state-names --json`** (#852 — 0.19.0) prints the published top-level name list of a project's `.local/xezar/` (section 3), so an external check reads it from the installed package instead of keeping a hand copy that goes stale on the next release. **Its stdout is a byte surface, not merely a shape**: exactly `JSON.stringify(STATE_NAMES_PAYLOAD, null, 2)` plus one trailing newline, and `packages/xezar/src/__fixtures__/local-xezar-top-level-names.expected.json` is the committed copy of those bytes — `local-xezar-top-level-names.test.ts` fails on any drift and `state-names-cli.test.ts` compares the real command's output against the same file. So **a change to the fixture IS a change to this surface** and is documented here; adding a name or a suffix is additive, and removing one, renaming a key, changing the indentation or dropping the trailing newline is breaking. The payload is GENERATED from the module and never read from that fixture, because the fixtures directory is not emitted into the package. Four more things are the contract: standard output carries the payload and nothing else (no mode line, no first-run notice, no banner), so a caller may pipe it straight into a parser; the command reads no project and writes no file, in any layout; `--json` is this command's own flag, a global flag before the word (`xezar --repo <dir> state-names --json`) changes nothing — the same bytes, still no read and no write — and any other option or word after it is refused; and exit is 0 when it printed and 2 for a usage error, with the usage line on stderr. Without `--json` it prints a table for a person, which is explicitly NOT a contract and may change freely.
...
- Workspace: `GET/POST /api/v1/projects`, `PATCH /api/v1/projects/:projectId`, `DELETE /api/v1/projects/:projectId`, `POST /api/v1/projects/checkout`, `GET /api/v1/fs/browse`, `GET /api/v1/models`, `GET /api/v1/providers/status`, `POST /api/v1/providers/connect`, `PUT /api/v1/providers/:provider/enabled`, `POST /api/v1/providers/:provider/retry` — the registered-project, filesystem, host model-catalog, and provider-authentication surface. The projects GET shape is `{projects: [{id, name, root, addedAt, lastOpenedAt, source, status, branch?, forge?, repoUrl?, maxParallel?, tags?}], bootProject, projectsDir}` (`status`: `ok`/`missing`/`not-git`; `branch` only when cheaply readable; `forge?` is the additive per-project forge classification (pre-rename issue 698) — `'github'` when the root's remote parses to a known forge host, omitted otherwise, so an old consumer that ignores it sees no change; `maxParallel?` is the additive per-project concurrency cap — omitted means "inherit the workspace cap", so an old consumer that ignores it sees no change; `repoUrl?` is the additive, credential-free web root of the project's remote (`https://github.com/owner/repo`, rebuilt from the PARSED remote so a token in it can never reach a client), which is what lets a cross-project surface link a reference the run knows only by number; `tags?` is the additive grouping-label list the global Tasks page filters and groups by — normalized (trimmed, deduped case-insensitively, sorted) and **omitted rather than `[]`** for an untagged project, so an old consumer that ignores it sees no change; `instance?` (#467, 0.17.0) is the additive DERIVED liveness answer `{state, url?}` described under [Instance mode](#instance-mode--which-projects-one-process-serves-0170-467) — checked per request, never read from the stored `lastListen` hint, and **omitted rather than null** when this server did not look, which is hosted mode). `PATCH /api/v1/projects/:projectId` is additive and **per-key**: body `{maxParallel?: 1..16 | null, tags?: string[] | null}`, each field applied ONLY when the body names it, so the pre-tags `{maxParallel}` body still means exactly what it always did and a tags-only body cannot clear a concurrency ceiling. `null` clears either (an empty `tags` array clears too); an empty body is refused with a 400, as it always was. The answer is `{project}` in the same entry shape. **The agent-account selection is deliberately NOT here** — it lives in `~/.xezar/agent-accounts.json` and is written through `PUT /api/v1/workspace/agent-profiles/selection`, so this route and the project registry are untouched by that feature. Same-origin only — no CORS, which is exactly what licenses the absolute `root`s that health must never carry. The list never 404s: an empty or unreadable registry answers `projects: []`.
  - **`GET /api/v1/models?runner=` — additive `local` / `vision`, 0.18.0 (#819 item 4).** The response
    shape `{runner, models: [{id, label, description}], source: live|cache|unavailable, stale, reason?}`
    is unchanged, and so are the cache, the probes and every existing field; a model option MAY now
    also carry `local?: boolean` and `vision?: boolean`. Both are present ONLY where the runner's own
    data proves them — `vision` from Codex's `inputModalities` and pi's per-model `input`, `local`
    from pi's provider `baseUrl` (a loopback, private or link-local IP literal or `localhost` is
    `true`, any other IP literal `false`, a host name unknown) — and ABSENT means unknown. Claude and
    OpenCode prove neither today, so their options carry neither key. Breaking: serialising `false`
    for an unknown value, deciding `local` from anything but the parsed host (a `127.` string prefix
    matches `127.evil.example`), or changing `id` from exactly what that runner's `--model` takes.
    The MCP `project_config` action `list_models {provider?}` is additive beside it: one row per
    tool, `{tool, available, source, stale, reason?, models}`, each the route's own answer with
    `available = source !== 'unavailable'`, an unavailable tool answered as a row with its reason
    and never dropped. It is a read — no operation key, no audit row, answered in hosted mode like
    the rest of the read surface — and `get_capabilities` is untouched: same three keys, no model
    catalog read. An older engine answers `list_models` with an invalid-option argument error, which
    is how a leader detects it; removing or renaming the action, or folding it into
    `get_capabilities`, is breaking.
...
- Agent accounts (additive): `GET/POST /api/v1/workspace/agent-profiles`, `PATCH /api/v1/workspace/agent-profiles/:id`, `DELETE /api/v1/workspace/agent-profiles/:id`, `PUT /api/v1/workspace/agent-profiles/selection`, `POST /api/v1/workspace/agent-profiles/import-global` (#819 — 0.18.0, below) — extra config dirs for a second login of the same agent CLI (`CLAUDE_CONFIG_DIR` / `CODEX_HOME` / `PI_CODING_AGENT_DIR`; pi joined on 2026-09-12, #329 — the entry that said it had no such variable was wrong). Workspace-level and single-mount. `GET` answers `{editable, profiles: [{id, provider, label, configDir, path, exists, looksValid, isDefault, status?, files}], profileCapableProviders, selections, defaults, problems}` (`files` is that agent's user-scope config files resolved inside THAT account's folder). **`status` is absent until the probe has warmed** and the listing never spawns a CLI to fill it — each probe is a shell-out to an agent CLI, one per provider plus one per account, which cost 2.5s on a real machine with four accounts. Absent means "not determined yet", which is NOT the same as the `unknown` a real probe can return; `GET …/:id/status` (optionally `?refresh=1`) is what actually probes, and the cockpit fills each row in from it. Provider auth for every account is warmed once at boot and kept in memory; reads are stale-while-revalidate (an expired answer is refreshed behind the response, never in front of it), a connected answer stands for minutes and a not-connected one is re-checked within a minute so a terminal login is noticed, the run gate re-verifies a provider before refusing, and anything xezar can observe (opening a login, repointing/removing an account, a runtime rejection) invalidates it explicitly, discovered defaults first (`id: "default"`, never stored, never deletable); `selections` maps a project's realpath'd ROOT to `{claude?, codex?, opencode?, pi?}`, and `defaults` is the same per-provider shape read as the machine-wide fallback for any repo that has chosen nothing (a repo's own selection always wins, which is what keeps it a default rather than an override). **Writing is a local-machine capability**: every mutator answers 409 in hosted mode (`XEZ_REMOTE`) and `GET` answers `{editable: false, profiles: [], selections: {}, defaults: {}}` there — the listing echoes absolute paths carrying the username, the same disclosure `/api/v1/health` trims. `DELETE` is deregistration only: the directory is never touched, and every reference to the removed id — in any project's `selections` AND in the machine-wide `defaults` — is scrubbed in the same atomic write (before 0.18.0 the scrub walked `selections` only, so deleting the account a provider defaulted to left a `defaults.<provider>` naming an account that no longer existed; #819 item 2).
  - Additive since 0.18.0 (#819 item 2): `GET …/agent-profiles` also answers `problems: [{kind: 'unknown-account', where: 'defaults' | 'selection', provider, handle}]` — the stored references that name no account, which the engine serves back and silently ignores at run time rather than repairing. `kind` is a literal and `where` says which map the handle came from; `handle` is the stored string as written. It is a REPORT, never an enforcement: an unknown id still resolves to the discovered account (`selectProfile`), which is the zero-config promise and is unchanged. The key is optional in the contract so the addition is additive for a typed consumer, and this route always fills it in (`[]` for a clean store and `[]` in hosted mode, where the store is never read). Nothing about this rides on `/api/v1/health`.
  - Additive since 0.18.0 (#819 PR 9): each `GET …/agent-profiles` row also carries `selected: boolean` — exactly one `true` per provider, the account the listing's subject runs under, resolved by the SAME function a run resolves through (`selectProfile`): in single-project mode this folder's own selection, then the default, then the discovered account; in the global layout the machine-wide default, then the discovered account. A dangling id therefore marks the discovered account, as a run would use it. The key is optional in the contract (the per-account `POST`/`PATCH` answers omit it, because one row cannot say which of its siblings is in use), and the listing always fills it in; hosted mode lists no rows at all, so it carries none. The cockpit's "In use" / "Default" marker reads it and never re-derives it. Nothing about this rides on `/api/v1/health`.
  - Additive since 0.18.0 (#819 PR 9): `GET …/agent-profiles` also answers `globalImport: {state: 'done' | 'declined' | 'unknown', importable}` — **in single-project mode on the host only**; the key is ABSENT in the global layout (there is no project to import into) and in hosted mode (the global home is never read there). `state` folds the per-machine outcome of `machine-state.json` (section 3): `imported` → `done`, `declined` → `declined`, and `not-asked` or no record at all → `unknown`, because neither is a "no". `importable` is a NUMBER — how many accounts the machine-wide file has that the project does not — and never a list: no id, label, provider or folder of that file is on the wire. It reads `0` when that file is absent or cannot be read. Absent is not `unknown`: a consumer draws nothing for a missing key. Never on `/api/v1/health`. Breaking: serving names or ids in place of the count, sending the key in hosted mode, or failing the listing when the machine-wide file cannot be read.
  - `POST /api/v1/workspace/agent-profiles/import-global` (#819 PR 9 — 0.18.0): the cockpit's and the leader's door to the SAME merge `xezar accounts import-global` runs (`importGlobalAccounts`), with the outcome recorded by the same rule (`globalImportStateAfter`: `imported` only when something was copied or the project already has accounts). No body. Answers `{added, kept, globalImport}` — COUNTS, never which accounts. **409 in hosted mode** (`capabilities().localHandoff` false), exactly like every other write of this family, and nothing is read or written there; 409 in the global layout ("nothing to copy"); 409 for a state file that is a symbolic link or an accounts file that cannot be read, with nothing written. Idempotent: a second call adds nothing and rewrites no bytes. Audited as `account.importGlobal` at both doors; the MCP action `project_config import_global_accounts` dispatches this route (its own entry in the MCP section below). Breaking: accepting it in hosted mode, making it overwrite a row the project has, or answering with account names.
...
- Agent quota (additive): `GET /api/v1/workspace/agent-quota` and `POST /api/v1/workspace/agent-quota/refresh` — the workspace-level, single-mount quota surface described in the #867 S2/S3 entries below. Both are readable in local and hosted mode. GET serves immediately and re-checks stale rows in the background (`wait=1` waits at most 20 seconds); POST explicitly requests the same bounded check. Neither response carries a folder or identity.
...
- **`machine-state.json` → `globalImport`** (#819) — optional, additive, and per machine on purpose: it records what THIS checkout did about the one-time import of the global setup (`imported`, `declined`, `not-asked`), so three answers that used to leave identical disk state can be told apart. **Absent reads as `unknown` and never as `not-asked`**: a file written before this field existed says nothing about consent, and claiming "nobody was asked" from an absence is exactly the fail-open reading the field exists to end. A value this version does not know degrades to `unknown` per key and never evicts the rest of the file. An older xezar ignores the key; a downgrade that re-serializes the file drops it, and a dropped value must be read as `unknown`. Breaking: defaulting an absent value to any of the three, or moving the record into the committed `workspace.json`, which every clone of the repository shares.
...
- **`.local/.gitignore`** — maintained by `ensureDataGitignore` in `packages/xezar/src/index.ts`. Everything the engine writes lives under `.local/`, so one blanket `*` rule covers every present and future run-data file; a new state file outside `.local/` would need its own rule and should not exist. The ignore rule settles git only: at least one external consumer checks the files at the top level of `.local/xezar/` against today's names and fails a gate on an unexpected one. Those names are a shape, not an enumerable set: during ordinary operation, with no feature turned on, any of them may also carry a `.lock`, `.lock.takeover` (the guard every lock release takes), `.tmp` or `.<pid>.<hex>.tmp` suffix or a rotation number `.1`–`.4`. New state therefore goes in a subdirectory, and a new top-level file is announced in the changelog. That is a known dependency, not a promise this section makes.
...
Users commit these files (`.xezar/workflows/*.yaml`) and share them across repos — the compact `skills:` form exists specifically to be portable. Protected shape: `name`, `description?`, and `steps` XOR `skills`; per step `id`, `name?`, agent fields (`prompt`, `skill`, `model`, `runner`, `allowedTools`, `bashAllowlist`, `timeout`, `verdictRole`) XOR check fields (`command`, optional `resultScope: routine | stage`, `onFail: {retry, max}`); the `{{task}}` token; `onFail.retry` referencing an earlier step; the built-in `quick-task` name.
...
`verdictRole` (#851) is additive and agent-only: `code-review`, `design-review`, `qa` or `architecture-review`, the role whose reviewer packet this step may have recorded. Absent is the zero-config default and means the step reports no verdict — which is now ENFORCED: a packet it leaves is refused (§3). A workflow meant to record verdicts must therefore declare its role, and the kit's `code-review`, `design-review` and `qa` workflows do; a step carrying the key cannot use the compact `skills:` form, and the cockpit builder, the save routes and a planned chain (`POST /api/v1/plan`, which offers the role on a reviewing step and strips an unknown one) keep it. Accepting it on a check step, widening it beyond the contract's role list, or letting an absent value match any role is breaking.

`timeout` (#22, 0.13.0) is an earlier addition to that list and additive by construction: it is optional, no file that predates it names it, and an ABSENT `timeout` is pinned to the exact spawn it produced before the field existed — the workflow's last (interactive) step uncapped, every earlier agent step falling through to the runner's 30-minute `DEFAULT_RUN_TIMEOUT_MS`. That default is now a protected surface in its own right: changing what an absent `timeout` does is a break even though no key changed. Its grammar is protected too — `<digits><s|m|h>` and the literal `none` — so a workflow written for 0.13.0 keeps loading. `none` is the only spelling for "no cap"; a zero-length duration and anything past Node's timer ceiling (`596h`) are refused at load time, and widening either later would be additive, not breaking.
...
The default-on team skills updater is an owner-approved exception to the normal opt-in rule for network and external-file mutation. Its compatibility boundary is deliberately narrow: work begins only after listen; checks are six-hour cached and bounded; updates name only sorted, lock-proven `qodeca/xezar-skills` entries, run serially under two stale-recoverable cross-process locks — the PROJECT half at `<cacheDir>/skills-update.lock` (`~/.cache/xez` globally, `<project>/.local/xezar/cache` in single-project mode) and the machine-wide `~/.agents` mirror at `<home>/.agents/.xez-skills-update.lock`, taken in every layout because that mirror never moves with the project — and failures never reject startup.

The gate lease's slot files (#672, `xezar lease` in section 1) follow that machine-wide half and not the layout: `<home>/.cache/xez/gate-slots/gate-slot-N.lock`, in every layout. That is the whole point of the feature rather than a detail of it — the contention it bounds is one disk, one page cache and one pool of test workers, so two single-project folders on one machine must contend, and a lock that moved with the state would give each of them its own slots and prevent nothing. Moving these under `xezCacheDir()` is breaking in the quiet way: everything would still render and test green.

Two consequences of that asymmetry are stated here rather than discovered. **The slots are machine-wide while the NUMBER is not**: in single-project mode `gateSlots` is read from that project's own committed `.xezar/workspace.json`, so two single-project folders on one machine with different numbers sweep different-sized slot sets — one on `1` and one on `2` can run together, because the smaller sweep only ever looks at slot 1. That is the honest reading of "the value is applied exactly as written" in the mode, not a defect to fix in the lock path; a person who wants one number for the machine writes it in both files, or runs the global layout. And **a machine that SLEEPS mid-gate for longer than the 60-second stale bound can hand a live holder's slot to a waiter** — after wake the wall clock has jumped, the holder's next heartbeat is up to 10 seconds away and the waiter sweeps every 2 seconds. The outcome is two overlapping gate runs, which is the pre-#672 behaviour, so it fails soft in the direction this feature came from; it is not the "two holders" the lock promises against in any other circumstance, and it is written down instead of being carried silently.

`XEZ_SKILLS_AUTO_UPDATE=0` (or the stored global override) restores detection-only behavior. Broad scope-only updates, invoking `gh`, updating untracked/manual skills, or making this work boot-blocking are breaking changes.
...
0.17.0) is the per-launch answer for the other side: it resolves the global
...
- **The one-time import is the one deliberate exception to "never opened" (BR-2, #600 part 5).** The
  ...
  afterwards. **Since #819 the exception has three doors and one rule** (a PERSON consents before the
  global home is read; nothing else may trigger it): the prompt, unchanged; the `--import-global` /
  `--no-import-global` flags, which are that person's answer typed on the launch and therefore
  replace the question rather than adding a way to skip it; and `xezar accounts import-global`,
  which a person types later and which merges accounts only. All three still go through the single
  allowlisted call site, and no environment variable reaches any of them. What each door decided is
  recorded per machine in `<project>/.local/xezar/machine-state.json` (section 3), never in the
  committed `workspace.json`. The exception is held to one call site: `packages/xezar/src/state-path-scan.test.ts`
  ...
  name, with its reason. Breaking: reading the home without a person's own yes — a prompt answered,
  a flag typed on the launch, or the `accounts import-global` command typed later, and nothing else,
  an environment variable least of all; writing to
  it, copying the registry or another folder's selection, overwriting a project file, writing outside
  the project (including through a symbolic link), or asking where nobody can answer. Until #819 the
  first run was also the only MOMENT the home could be read; that is now the flag's rule rather than
  the exception's, and the consent rule is unchanged.
- **The cockpit's count is a second, narrower read of the global home (#819 PR 9, owner decision
  "Allow the count", 2026-09-21).** So the Agent accounts pane can offer "Copy {n} accounts",
  `countImportableGlobalAccounts` (`workspace/import-global.ts`) reads the global `agent-accounts.json`
  on each accounts listing in single-project mode — without a person's yes, which is why it is its
  own entry. Its limits are the contract: it returns a NUMBER and nothing else (no id, label,
  provider or path leaves the function); it is read-only (it never writes, never creates the global
  home, never migrates a file); an absent, unreadable, unparsable or permission-denied file reads as
  `0`, never an error and never a warning; it runs only in single-project mode on the host (never in
  hosted mode, never in the global layout); and it never reaches `/api/v1/health`. It has its OWN
  allowlist line in `state-path-scan.test.ts`, separate from the import's. The copy itself still
  needs an explicit request: the prompt, a launch flag, the CLI command, the cockpit's "Copy" button
  or the leader's `import_global_accounts` (both `POST /api/v1/workspace/agent-profiles/import-global`,
  section 2) — the owner allowed the leader on 2026-09-21 ("Allow both, people and MCP (leader) to
  use the import my accounts functionality"). Nothing copies on a read or on a listing. Breaking: returning anything but a count, writing to the global home, reading
  it in hosted mode, or letting an unreadable file fail the listing.
...
## The MCP `project_config` tool copies the machine-wide accounts (#819 PR 9) — additive, 0.18.0

**Owner, 2026-09-21: "Allow both, people and MCP (leader) to use the import my accounts
functionality."** So a project leader can run the same copy a person runs with
`xezar accounts import-global` or the Agent accounts pane's "Copy {n} accounts". This entry is
separate from the route's (section 2) because it is the leader's door.

- **The action.** `project_config` action `import_global_accounts`. Arguments: `operationId`
  (required, like every write of this tool) and nothing else — any other key is refused as unused.
  It dispatches `POST /api/v1/workspace/agent-profiles/import-global`, the route the cockpit posts,
  so the merge is `importGlobalAccounts`: merge-only, it never replaces an account, a default or a
  selection the project has, never writes a dangling default, and a second call adds nothing.
- **The answer.** `{added, kept, globalImport: {state, importable}}` — counts and the import state,
  never which accounts (no id, label, provider or folder of the machine-wide file).
- **Refusals** are the route's own, in its own words: 409 in hosted mode, 409 in the global layout
  (there is no project to import into), 409 for a state file that is a symbolic link or an accounts
  file that cannot be read. Nothing is written on any of them.
- **Audit.** One action id, `account.importGlobal`, at both doors (`mcp/audit-inventory.ts`), so a
  leader's call and a person's click land on one record shape.
- **The read half.** `get_account` additively carries `globalImport` — the listing's own field,
  passed through as it is, and absent whenever the listing omits it (the global layout, hosted
  mode). A leader reads absence exactly as the cockpit does: not "unknown", but "nothing to import
  into here".
...
- **MCP at S2:** `project_config read_quota` originally dispatched the GET without starting a
  check. S3 below adds bounded stale refresh and the separate `check_quota` action.
...
## Agent quota engine checks (#867 S3) — additive, 0.19.0

- **HTTP:** `POST /api/v1/workspace/agent-quota/refresh` accepts the strict contract body
  `{provider?, accountId?}`. It is chained into the workspace agent-profile family, is allowed in
  hosted mode, and remains protected by the existing same-origin mutation guard. GET additionally
  accepts `wait`; `wait=true` (and the internal compatibility spelling `wait=1`) waits for a stale
  background check, and the wait is bounded.
- **MCP:** `project_config check_quota` dispatches the POST with the same optional selectors.
  `read_quota` dispatches GET with `wait=1`. Both return the shared quota answer in `result`, work
  in hosted mode, and carry no path, config directory, email or organisation.
- **Bounds and lifecycle:** checks begin only after the server listens. One login is checked at
  most once per five minutes, no more than two login checks run concurrently, and a check or
  waiting caller is bounded to 20 seconds. Rows older than 15 minutes are re-checked only while a
  WebSocket subscriber is present or a GET/MCP read was seen in the previous 15 minutes; the
  one-minute scheduler stops when neither condition holds. State remains process-memory only.
