...
The gate lease's slot files (#672, `xezar lease` in section 1) follow that machine-wide half and not the layout: `<home>/.cache/xez/gate-slots/gate-slot-N.lock`, in every layout. That is the whole point of the feature rather than a detail of it — the contention it bounds is one disk, one page cache and one pool of test workers, so two single-project folders on one machine must contend, and a lock that moved with the state would give each of them its own slots and prevent nothing. Moving these under `xezCacheDir()` is breaking in the quiet way: everything would still render and test green. `XEZ_SKILLS_AUTO_UPDATE=0` (or the stored global override) restores detection-only behavior. Broad scope-only updates, invoking `gh`, updating untracked/manual skills, or making this work boot-blocking are breaking changes.
