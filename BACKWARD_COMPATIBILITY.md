...
- **The stored key is editable, and the edit says so** (0.17.0, PR 5). `cli.instance` is written
  through `PUT /api/v1/workspace/config` (Settings → Terminal, global scope) and through the MCP
  `project_config` action `set_workspace_config`, both validating with the same contract schema, so
  the two doors produce the identical `~/.xezar/config.json`. `null` clears the key back to the
  `XEZ_INSTANCE`/`workspace` chain, and a body that does not name `cli` leaves it byte-identical on
  disk. Because the mode cannot be re-taken live, the control states that it applies at the next
  start and a narrowed cockpit is told the narrowing wins here instead of being shown a stored value
  that is not what it is doing; `cli.inForce` is what drives that, and answering it from the stored
  value is breaking. `output`, `color` and `logLevel` remain file-and-environment-only.

**This entry discharges the "What is not in this entry" paragraph above.** That paragraph said the
one-cockpit-per-project default and `--instance workspace` were a separate change; the separate
change is this entry, it shipped with `workspace` as the default rather than the opposite one the
2026-09-16 Q2 decision anticipated, and with PR 5 it is editable from both doors. The paragraph
stays as written because it records what was true for 0.16.0.
