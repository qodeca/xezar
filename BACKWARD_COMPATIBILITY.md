...
- **Commands:** bare invocation = `serve` (cockpit); `xezar run "<task>"`; `xezar init`; `xezar mcp` (the stdio MCP bridge a coding agent starts); `xezar lease gates -- <command>` (#672 — unreleased; ships in 0.17.0); `xezar server-install` / `server-deploy` / `server-uninstall` (the hosted-instance provisioner).
...
mode](#instance-mode--which-projects-one-process-serves-0170-467). The short version: `--instance`
exists, `project` is opt-in, and `workspace` — every registered project in one cockpit — remains
the default, so nothing in the paragraph above describes behaviour a 0.16.0 user loses.
