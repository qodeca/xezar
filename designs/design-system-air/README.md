...
- **(h) Shell breakpoint.** The replica switches to the phone shell at 860 px, which was `cockpit.css`'s breakpoint when this mockup was drawn, not the app's `md` at 768 px; the review widths, 1280 and 375, sit clear of both. Since 2026-09-18 `cockpit.css` switches below 768 px (#447 OD-3) while this folder's own sheets keep 860, so a window between 768 and 860 px mixes the two; 1280 and 375 are unaffected.
