...
| [cli-terminal](cli-terminal/README.md) | In review – terminal output (live activity, settings, ports) and one cockpit per project, for #467; text mockups plus the “Other projects” switcher; the Settings → Terminal pane of PR #798 failed design review at `b659c16a` (B-1, B-2) and its round-1 response awaits re-review |
...
| [single-project-mode](single-project-mode/README.md) | Approved – the cockpit surface of single-project mode (the mode badge, Settings naming its file, the unavailable account, the three removals and the refused registry page) for PR4 of qodeca/xezar#600. Design review PASS WITH FOLLOW-UPS on PR #604 (`design-approved`); NB-1 through NB-5 each carry a disposition in `single-project-mode/README.md` § Design review. Partly implemented: PR #611 merged; PR #612's four copy items passed a scoped design review (`design-approved`); Implemented once #612 merges. Six open decisions in `single-project-mode/README.md` §15 and the owner's unconfirmed answers in §16 remain open |
| [agent-accounts-onboarding](agent-accounts-onboarding/README.md) | Draft – Settings → Agent accounts shows every account per agent at once, “In use” and the built-in login in words, unknown-account problems with their fix, and the import state with a person-only “Copy N accounts” action (#819 PR 9); six open decisions in `agent-accounts-onboarding/README.md` §13; waiting for the first `design-review` |
| [agent-quota](agent-quota/README.md) | Draft – plan limits for every Claude Code and Codex login in Settings → Agent accounts (a limits half per row, a Plan limits block) and a limits chip above the sidebar footer and in the phone top bar, every state including stale, out, unknown, hosted and empty (#867 S4); twelve open decisions in `agent-quota/README.md` §13; waiting for the owner's look (D34) and the first `design-review` |
| [design-system-air](design-system-air/README.md) | Implemented (PRs #429, #432, #431, #437, #438, #441, then #574 and #585 for the Git, GitHub, Compare and Automations gutters) – rhythm tokens, looser between-block defaults and Roomy density; #424 reopened 2026-09-17 for those pages; allowlist target met at two WCAG floor rows (#445, closed 2026-09-18); phone drawer target: #430 |
