...
| **Whole `npm run test:e2e` suite passing in shuffled order** | browser e2e | Not yet provable: ~19–20 of the 45 pre-existing e2e files (owned by the UI lane, outside this guide-flow package's authority) are themselves order-dependent, independent of the new `guide-*.e2e.ts` files. Accepted proof for this package is narrower — each new file alone, the new files together in shuffled order, and the whole suite in default (CI) order — recorded in `.local/xezar-tasks/7d26c8f0-31e4-4831-9236-d709fe8f68dc/evidence/BLOCKED` and `evidence/existing-suite-shuffled-same-seed.log` (primary checkout). Shuffled-order clean-up of the pre-existing files is browser-test pull request 4's job | **P** |
