...
| The `serve` wiring that gives `followProjectDoors` its real lifecycle (`index.ts`'s `onContexts` hook opens each late-built project's own MCP socket through `open`, and shutdown closes every door) (#557) | saved deterministic harness, not a unit suite | `scripts/multi-project-harness.mjs` (`npm run test:multi-project`) is the only suite that exercises this wiring; the lifecycle itself (build/dispose ordering, retiring, `onOpened`) is unit-tested in `mcp/project-doors.test.ts` | P (2026-09-18) — not run in CI |
