...
| `.xezar/pipeline/config.json`, `.xezar/pipeline/trackers/`, `scripts/` | Maintained pipeline config, tracker descriptor and repository scripts. The kit contract test and the optional `xez-*` team collection read the `.xezar/pipeline/` paths; their outputs do not live there. |
| `docs/features/` (the `specs` path declared in `.xezar/pipeline/config.json`), `docs/testing/agent-browser.md` | Maintained specifications and browser contract. A new specification written there is maintained source, not an ignored working draft. |
| `.local/xezar/` | Engine run index, transcripts, attachments, handoffs, todos, UI state, automation state/logs/locks, temporary run directories and Git worktrees. Git ignores everything here, but at least one external consumer checks the top-level file names against the ones the engine writes today (`runs.json`, `machine-state.json`, `audit.ndjson` and the rest). Those names are a shape to match, not an exact list: on a healthy project with no feature turned on, any of them may also carry a `.lock`, `.tmp` or `.<pid>.<hex>.tmp` suffix or a rotation number `.1`–`.4`. Put new state in a subdirectory, and announce a new top-level file in the changelog ([project layout](../project-layout.md#discovery-and-writes)). |
...
unit/package-test temporary directories, and the `.ai` directory names in use before the layout move (#11). Historical
