...
- **A xezar tool gated behind the extension’s `approveTools` is answered by xezar’s pi runner since [#369](https://github.com/qodeca/xezar/issues/369).** The gated call opens the extension’s approval dialog (`extension_ui_request`, `method: "select"`), which carries no `timeout`, so pi’s `docs/rpc.md` says it blocks until the RPC client answers it. Until #369 nothing in xezar did, and #330 WP5’s QA measured the consequence through a real `xezar serve` (2026-09-12): an ordinary pi task was **unaffected, done in 2.8 s** with the gate on, because the runner’s default `--tools` allowlist offers the model no `xezar_*` tool and the dialog never fires; a step whose own `allowedTools` named `xezar_health` **failed at 121 s** with `pi CLI timed out after 2m and was killed`; the same step with no gate finished in 3.0 s. The runner now answers: an autonomous run refuses at once with `Deny` and records it (the model is told the call was declined, the turn ends), and an interactive run shows the dialog in the task as a question card with pi’s own three choices and routes the answer back by the dialog’s id — the A-01 `approveTools` leg of `packages/xezar/test/integration/mcp-real-clients.test.ts` drives a real pi through the runner in both modes. A person at their own pi TUI answers the dialog themselves. `approveTools` is the user’s own key in the adapter’s `mcp.json` — `settings.approveTools`, or the per-server key that overrides it — so the zero-config default is unaffected. What stays open: a headless pi that some other program drives over `--mode rpc` is that program’s to answer, and the leader extension, which runs inside pi, answers no dialog.
