...
| `uiState` | object | no |  | set_workspace_ui_state: the shared presentation preferences to change — they apply to every project on this machine. Only the keys you send are touched: appearance (accent, density, width), notifications.enabled, taskTable.expandedColumns, importedSkills (the whole curated list) and dismissedProviderAuthFailures. The colour theme is not here: it is stored by the browser itself, not by the server. |
...
| `importedSkills` | array of string | no |  | import_skills: the WHOLE curated list of default skill names to show, replacing the previous one; [] shows none of them. Read the names with list_importable_skills. |
...
| I-024 | Decided 2026-09-20 (D-677-B3, owner rule "every key", exclusions of 07:41). | `project_config:set_workspace_ui_state` |
...
| I-092 | Decided 2026-09-20 (D-677-B3, owner rule "every key"). | `project_config:list_importable_skills`, `project_config:set_workspace_ui_state`, `project_config:import_skills` |
...
| I-132 | Decided 2026-09-20 (D-677-B3, owner rule "every key", exclusions of 07:41). | `project_config:set_workspace_ui_state` |
