...
| § 4.16 the terminal settings (`cli.instance`, `cli.output`, `cli.color`, `cli.logLevel`) | workspace-write | `set_workspace_config` via `PUT /workspace/config`, read by `get_limits` — #467 PR 5; the three presentation keys by the owner's D-5 of 2026-09-20 | Previously: not classified at all — the keys had no Settings control and no write door, so no cockpit action named them. |
...
> **SUPERSEDED, 2026-09-20 (#677 wave 2 slice B5).** The owner decided that agent accounts are
> "**Writes and identity read**" (07:41). The per-project selection is a leader write now
> (`select_account`), the account administration rows below it are leader writes, and the account
> identity this decision put outside the MCP boundary is served by `get_account_details` alone.
> The machine-wide "Defaults for new projects" half is the one thing still out of reach, and not
> by a refusal: a leader has no project id argument, so `projectId: null` cannot be sent. The
> decision is kept verbatim below as a dated record of what was true then, per § 10.
>
> **What is withheld did not shrink except where the owner said so.** The selection answer is
> still narrowed to the bound project, the account row is still narrowed (no `path`, no `files`),
> and a label that looks like an email is still withheld — the identity read is the ONE reversal.
...
| `modelsLocked` (write, project key) | project-write | **#677 C2, owner 2026-09-20: "Both doors, like every key" (the self-disarm hole is accepted).** `set_config` writes this project's own key through `PUT /config`; `true` stores it, `false`/`null` delete it. It cannot lift `XEZ_AGENT_MODELS_LOCKED` or the workspace key, and a body that clears the lock AND sets `defaultModels` is still refused 409 because the guard reads the lock before the write. Hosted mode allows this write like every other `PUT /config` key (#734). | E-BIND, E-CONTRACT (`setConfigInputSchema.modelsLocked`) | — |
| `projectModelsLocked` (read) | project-read | This project's own key, so a reader can tell a lock it can lift from one it cannot. | `configAnswer` (`server.ts`) | Which of the environment, the workspace config or a user-scope file holds any other lock. |
...
| Default agent picker — ACCOUNT half (`useSelectAgentProfile` → `PUT /workspace/agent-profiles/selection`) | safe-effective-read | **SUPERSEDED — the per-project write is reversed, 2026-09-20 (#677 B5, owner decision 07:41); see the superseding table in section 1 for the current classification.** `select_account` writes THIS project's selection through the cockpit's own route; the machine-wide default is still out of reach, because a leader cannot name a project. Previously: **D-03-3.** Writes a global personal file and names an account identity. | E-409-PROFILE, E-BIND, E-NARROW; read through `selectionFor` (`agent-accounts.ts:280`) | Email/login/credential, `configDir`/`path`, other projects' selections, the machine-wide `defaults` map. |
...
| Providers card — Try again (`POST /providers/:provider/retry`) | excluded | **SUPERSEDED — reversed 2026-09-20 (#677 B4); see the superseding table in section 1 for the current classification**, under the same owner rule: `clearRuntimeAuthFailure` still clears a workspace-wide incident, and the leader may now clear it. A leader CAN clear an incident it never saw: the tool reads the current `authFailureId` from `GET /providers/status` inside the handler, because F-03 keeps that id out of every leader-facing answer and therefore out of its arguments (#760 review, Minor 2). What the route's guard still protects is narrower: a rejection arriving BETWEEN that read and the write is a different id, and the route answers its own 409 instead of erasing it. | `retry_provider` → `POST /providers/:provider/retry`; `ProviderAuthService.clearRuntimeAuthFailure` | The `authFailureId` itself, in both directions: it is never served and never accepted as an argument. |
...
| Per-task memory limit (`ProjectMemoryLimitField` → `putConfig({memoryLimitMb})`, added 2026-09-21 by #677 C1) | project-write | The cockpit control for the per-repo key already classified in § 4.14 (`setConfigInputSchema.memoryLimitMb`); rendered in every mode because `PUT /config` is not a registry door. | E-BIND, E-CONTRACT (`setConfigInputSchema.memoryLimitMb`) | The workspace-level `resources.memoryLimitMb`, which is never writable from here. |
...
| `resources.gateSlots` (Gate slots) | workspace-write | How many full gate runs the machine-wide gate lease admits at once (#672). **Machine-wide in effect and not a per-project number**: the slot files live under `<home>/.cache/xez/gate-slots/` in every layout, so raising it lets every checkout on this box run gates together. Given a cockpit control by #672 G4, so both doors write the key the owner's 2026-09-19 rule requires. The read is the EFFECTIVE count — the file key is optional, and an absent one derives 1 — and `null` on a write CLEARS it back to that derived default rather than storing anything. | `setWorkspaceConfigInputSchema.resources.gateSlots` (`packages/contract/src/workspace.ts`, 1–16 or `null`) at both doors; `WorkspaceSemaphore.gateSlots()` (`semaphore.ts`); read fresh per gate run by `xezar lease gates` | Whether any gate run is holding a slot right now, and which checkout holds it — that is other-project activity (N-01). The leader reads the ceiling, never the occupancy. |
...
| Field / action | Status | Reason | Enforcing code path | Withheld |
| --- | --- | --- | --- | --- |
| Effective account for the bound project (read) | safe-effective-read | **D-03-3**, and the read half of `select_account` since #677 B5. Reported as a local profile **handle** and a **display label**. | E-409-PROFILE, E-BIND, E-NARROW; `selectionFor` (`agent-accounts.ts:280`) | Email, login, credential; `configDir` and `path`; other projects' selections; the machine-wide `defaults` map; every other account row. |
...
| Per-project selection write, and the machine-wide default write (`projectId: null`) | excluded | **SUPERSEDED — reversed 2026-09-20 (#677 B5, owner decision 07:41: accounts are "Writes and identity read"); see the superseding table in section 1 for the current classification.** The per-project half is `select_account`, through `PUT /workspace/agent-profiles/selection`. The MACHINE-WIDE default half is NOT reversed and stays person-only — not by a new refusal, but because a leader has no project id argument at all (D-01 § 1.5), so the bound project's id is supplied and `projectId: null` can never be sent. Previously: **D-03-3.** A global personal file, naming an account identity. | `server.ts`; E-409-PROFILE | The `selections` map's other roots — the answer is narrowed to this project's own selection. |
| Add account (`POST /workspace/agent-profiles`) | excluded | **SUPERSEDED — reversed 2026-09-20 (#677 B5, owner decision 07:41: accounts are "Writes and identity read"); see the superseding table in section 1 for the current classification.** `create_account`, through the cockpit's own route. **The accepted exposure, recorded rather than mitigated:** `configDir` becomes an agent CLI's whole home, an agent home can hold settings and hooks that run when the next task starts that agent, and nothing validates that a folder is a sane one. No new validation was added here on purpose — a second opinion about the filesystem is how two doors drift — so the mitigations are the route's own bounds and E-409-PROFILE, which refuses the whole family in hosted mode. Previously: creates a second login on the machine and points an agent at a host directory. F-12, M-22. | `server.ts`; E-409-PROFILE | The row is narrowed: no expanded `path`, no `files`, no `status`, and a label that looks like an email. |
| Rename account (`PATCH /workspace/agent-profiles/:id`) | excluded | **SUPERSEDED — reversed 2026-09-20 (#677 B5, owner decision 07:41: accounts are "Writes and identity read"); see the superseding table in section 1 for the current classification.** `update_account`, which also repoints `configDir` — the same accepted exposure as Add account above. Previously: global account administration. | `server.ts`; E-409-PROFILE | The same narrowed row as Add account, **plus the stored `configDir` on a call that did not send one** (#764 review, Minor 2, 2026-09-20): a rename answers no folder, because the stored one is an absolute host path the leader never supplied. An update that repoints the folder echoes the folder it sent. |
| Remove account (`DELETE /workspace/agent-profiles/:id`) | excluded | **SUPERSEDED — reversed 2026-09-20 (#677 B5, owner decision 07:41: accounts are "Writes and identity read"); see the superseding table in section 1 for the current classification.** `remove_account`. Deregistration only — the folder and its credentials are untouched — but it is irreversible for every project, and the route's own scrub of every selection naming it happens in the same write. Previously: global account administration, irreversible for every project. | `server.ts`; E-409-PROFILE | — |
| Connect an account (`POST /providers/connect` with `profileId`) | excluded | A host terminal plus an account path. See §4.1. | `server.ts` | — |
| Re-check an account (`GET /workspace/agent-profiles/:id/status`) | excluded | **SUPERSEDED — reversed 2026-09-20 (#677 B5, owner decision 07:41: accounts are "Writes and identity read"); see the superseding table in section 1 for the current classification.** `check_account_status`, narrowed to the `get_capabilities` vocabulary. The `provider` in the answer is the ACCOUNT's, and a call naming an account with a different backend is refused rather than answered under the caller's word (#764 review, Nit 6, 2026-09-20). Previously: probes one named account's login; naming the account is the disclosure. | `server.ts`; E-409-PROFILE | The `authFailureId` and the `profileId`, as for every provider row. |
| **Show details (`GET /workspace/agent-profiles/:id/details`)** | excluded | **SUPERSEDED — reversed 2026-09-20 (#677 B5, owner decision 07:41: accounts are "Writes and identity read"); see the superseding table in section 1 for the current classification.** **This is the one place this programme deleted a NEGATIVE requirement**, on the owner's own word and against the spec's recommendation to keep it refused (§ 4 Q3). `get_account_details` serves exactly what this route serves, for one account the leader names; it has its own `BACKWARD_COMPATIBILITY.md` entry. Identity still reaches no other answer. Previously: **this is the account-identity read** — email, organization and similar claims from the agents' own auth files. F-12 and N-01 restrict it absolutely, and its module states it is answered to exactly one route, never joined, never logged. | `readAccountIdentity` (`packages/xezar/src/agent-config/account-identity.ts`); route at `server.ts`; E-409-PROFILE | Nothing this route returns; everything it does not (no join, no listing, no second source). |
| Open an account's folder or file (`POST /workspace/agent-profiles/:id/open`) | excluded | **NOT reversed by the 2026-09-20 decision** (#677 B5): it opens home-directory content in an application on the person's desktop, the `host-process` boundary Connect keeps, and the decision was about accounts rather than host processes. | `server.ts`; E-409-PROFILE | — |
...
New section, four keys of one stored object. They are classified `workspace-write` under the same
owner rule of 2026-09-20 ("every key") that § 4.9 records — the three presentation keys by the
owner's decision D-5 of the same day — and they are the first workspace writes that do NOT take
effect on the running process, which is the only thing that makes them different from their
neighbours, and it is stated in the answer and in the control rather than left for a leader to
discover.

| Field | Status | Reason | Enforcing code path | Withheld |
| --- | --- | --- | --- | --- |
| `cli.instance` / `cli.effectiveInstance` / `cli.instanceSource` / `cli.inForce` / `cli.narrowing` | workspace-write | Which projects one xezar process serves. Machine-wide in effect, and a fact about the leader's OWN cockpit rather than another project's data, which is why the read is not narrowed away. `inForce` is reported because the two registry narrowings beat the setting and a leader told only the stored value would be wrong about its own process; the cockpit also reads `narrowing` to tell them apart, and shows a folder that owns its state as read-only text. | `setWorkspaceConfigInputSchema.cli` (`packages/contract/src/workspace.ts`); the route's `mergeWriteWorkspaceConfig` branch, guarded per key on the body NAMING it; `resolveNextStartCli` (`packages/xezar/src/cli-settings.ts`); `instanceModeInForce` (`packages/xezar/src/workspace/projects.ts`) threaded in at boot | `instanceSource` and `narrowing` are the cockpit's copy drivers and are not in the `get_limits` answer, which carries `{effective, inherited, inForce}`. The answer carries no port, no bind address and no `lastListen`. |
| `cli.output` / `cli.color` / `cli.logLevel` and their `effective…` / `…Source` fields | workspace-write | How a xezar started from a terminal prints its activity. Presentation, machine-wide, applied at the next start. | the same schema and the same per-key route branch | The `…Source` fields stay cockpit-only; `get_limits` answers `{effective, inherited}` for each, as its other rows do. |
...
| --- | --- |
| Accounts | **SUPERSEDED, 2026-09-20 (#677 wave 2 slice B5; recorded 2026-09-20 after the #764 review, Minor 5).** The owner decided that agent accounts are "**Writes and identity read**" (07:41), so the first half of this row — "each is refused" — is no longer what the product does: selection, add, rename, remove, the sign-in check and Show details are leader writes and reads now, and only Connect and Open stay refused (§ 4.11 carries the per-row detail). The second half holds with one clause added and one clause narrowed: no response, error or event contains an email or a login — **including a refusal**, since the duplicate-folder 409 is redacted at the MCP door (#764 review, Major 1) — and an account PATH appears only as the echo of a `configDir` the same call supplied (#764 review, Minor 2). The cross-project half is unchanged and is what this row still tests: B's `~/.xezar/agent-accounts.json` entry is byte-identical after A's leader has done all of it, and nothing of B's reaches A. The decision is kept verbatim below as a dated record of what was true then, per § 10. <br><br> *Previously:* A's leader attempts every write in §4.11 — selection, add, rename, remove, connect, details, open — and each is refused. B's `~/.xezar/agent-accounts.json` entry is byte-identical afterwards, and no response, error or event contains an email, a login or an account path. |
