...
**`claude-code-push-not-seen`** (#886) — xezar pushed events to the attached Claude Code session more than five minutes ago, and the session has kept calling other xezar tools since without reading or acknowledging them. The pushed events are most likely not reaching the conversation. Claude Code does not confirm delivery, so this is what xezar can see, not a certainty. Nothing is lost: the events stay in the journal.

fix: Read the events now with leader_events action read. To see why Claude Code dropped them, start Claude Code again with --dangerously-load-development-channels server:xezar --debug-file <a file path>, attach again, and look in that file for "Channel notifications registered" or for "Channel notifications skipped:" and the reason after it. Until then, read events with leader_events.

> **2026-09-22 — #886, pushes reported missing on Claude Code 2.1.280.** A leader session reported no `<channel source="xezar">` message for eight delivered rows while `status` said attached and push-capable. Seven isolated live runs (Claude Code 2.1.278 and 2.1.280, a synthetic API-key mode with a dummy key and a scripted model endpoint, bridge from the published 0.18.0 package and from a source checkout, a 0.18.0 and a source cockpit, `stop` + `attach`, a background task running, the parent-session variables a `claude` started from another Claude Code session's shell inherits, and `remoteControlAtStartup`) all logged `Channel notifications registered` and delivered every event as a model turn. The bridge's frame matches the schema read from the 2.1.280 binary (`claude-channel-golden.test.ts`). The same test pins a bridge from a source checkout against the published 0.18.0 service by replaying that service's real `session/open`, tool and `leader/push` exchange, captured once from the wire and hash-pinned (`__fixtures__/xezar-service-0.18.0-wire.json`). The drop was not reproduced outside that leader's own account session, so which Claude Code condition dropped it is not known; the new `claude-code-push-not-seen` blocker and the debug-file check in `docs/guide/13-mcp-leader.md` make the next occurrence visible and diagnosable.
