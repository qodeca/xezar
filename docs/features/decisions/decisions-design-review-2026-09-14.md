...
| G-6 | Card send row | No third answer | F-15: "not a decision" closes the case and continues the task | Add a ghost "Not a decision" action in the send row; a decided variant "Not a decision · by you · {time}" |
...
| G-14 | README § 13 | Fifteen acceptance criteria written against `waiting` | § 9 of the requirements: A-1 to A-18 | Replace with a pointer; keep the design-only criteria (states, keyboard, themes) |
