...
| F-20 | `record_decision` MUST require `ownerWords` (non-empty, bounded at 1 000 characters) and stores `channel: 'mcp'`; on a `delegated` case (F-31) it requires `reason` instead and refuses `ownerWords`, so a relayed answer and a leader's own answer can never be confused in the record. The words land on the run's NDJSON and in the case; never in the audit trail (`audit.ndjson`, formerly `mcp-audit.ndjson`), which carries digests only. Whether the agent receives the words verbatim or only the option label is OD-6. A leader that opens and records the same case is allowed (non-goal) but the record shows both operation ids and the seconds between them. |
