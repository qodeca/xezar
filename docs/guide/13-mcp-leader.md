...
This was checked in isolated sessions of Claude Code 2.1.278 and 2.1.280 (an API-key login, a xezar cockpit in a scratch project): both registered the channel and received every pushed event, idle and after a `stop` and a new `attach`, with the bridge started from the published package or from a source checkout. A claude.ai subscription login, an organisation policy and Remote Control were not checked there.
