...
**How to check that pushes work.** xezar can see that it wrote a channel message to the session, but Claude Code does not confirm what it did with it, so an attached session and a rising `deliveredSeq` are not proof. Check the session itself:

1. Start Claude Code with the channel flag and a debug file, from the project root: `claude --dangerously-load-development-channels server:xezar --debug-file <a file path>`.
2. Call `health`, then `leader_events` `attach` with a fresh operation ID.
3. Look in the debug file. `MCP server "xezar": Channel notifications registered` means Claude Code will turn xezar's pushes into turns. `Channel notifications skipped:` is followed by Claude Code's own reason, for example `server did not declare claude/channel capability`, `channels are not available on third-party providers`, `channels not enabled by org policy`, or `server xezar not in --channels list for this session`. Claude Code writes that reason only to its debug file; xezar never sees it.
4. Wait for a real event, such as a task finishing. It must appear as a `<channel source="xezar" …>` message, and the debug file shows a `notifications/claude/channel` line for it. Only then is the session **delivery verified**.
...
**Tips and tricks.** Start the cockpit before the new Claude Code session so the bridge can advertise the channel. A `claude-code-bridge-too-old` blocker means the client must restart the current bridge. A `claude-code-push-unconfirmed` blocker does not mean the row was lost: inspect the session and journal, then acknowledge only after the event has been handled. A `claude-code-push-not-seen` blocker is stronger: xezar pushed events more than five minutes ago and the session has kept calling other xezar tools since without reading or acknowledging them, so the pushes are most likely not reaching the conversation. A session that reads its events with `leader_events` `read` is never reported this way for the events that read returned; read every page while `hasMore` is true, because a read that answers a gap or stops before a pushed event does not count as having seen it. Read the events with `leader_events` `read`, then use the debug-file check above to see why.
