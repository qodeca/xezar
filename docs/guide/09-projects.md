...
The project then keeps its setup in its own `.xezar/` folder: `config.json`, `workspace.json`, `agent-accounts.json` and `workspace-ui.json`, which you can commit. Working files stay in `.local/xezar/`, and xezar does not open `~/.xezar`. Nothing that describes your machine goes into the committed files: the time this clone was last opened and the port its cockpit last used live in `.local/xezar/machine-state.json`, so a launch leaves `git status` clean and teammates never conflict over them. You need the flag only the first time; after that the presence of `.xezar/workspace.json` decides, so every start in that folder, or in a clone of the repository, is in the mode. The terminal prints one line naming the mode and both folders. A linked Git worktree, a folder under `.local/xezar/worktrees/` and your home directory itself never enter the mode.
