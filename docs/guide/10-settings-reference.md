...
#### To read and refresh plan limits

The **Plan limits** block and each Claude Code or Codex login row show the limits those tools report, including the reading's age, reset times, plan details, and any reason a reading is unavailable. The three statuses have precise meanings:

- **Can work** (`ok`) means the tool reported that the login can work now.
- **Out until …** (`out`) means a reported plan limit is exhausted; the displayed time is when that login becomes available again.
- **Limits unknown** (`unknown`) means xezar could not read whether the login can work. It never means the login has budget. An API-key login's row instead reads **Limits not reported**; its status is still `unknown`, because API-key logins do not report plan limits.

Per-model weekly limits appear under the login when the tool reports them: Claude Code names the model (for example Opus or Fable), and Codex names the model slug it reports for that limit (for example `gpt-6-astra`). A Codex per-model limit describes that model only; it never changes whether the login can work. When a Codex task stops because the login ran out of its usage limit, that login reads **Out until …** at once, with the reset time Codex gave.

Use **Refresh** on one eligible login or **Refresh all** in the Plan limits block to request a fresh check. One login can be checked at most once every five minutes, so the control shows when its next check is allowed. Refresh is not offered when another check cannot help, including for an API-key login or a missing or unsupported tool version.

The plan-limits chip summarizes the same answer throughout the cockpit. On a desktop it sits above the sidebar footer and shows a dot and `can work / total` count for each applicable agent; on a phone it appears in the top bar as one combined count. Open it to see which logins are out or unknown and to jump to **Settings → Agent accounts → Plan limits**. The chip is absent until there is a readable answer and at least one installed Claude Code or Codex login is a subscription login – the login kind the agent itself reports for that login's credentials. An API-key login, or one whose kind the agent did not report, never counts; **Show details** on each row names its login kind. Xezar displays these facts but does not switch a login or hold a task back because of them. A project leader reads the same answer through [`project_config` quota actions](13-mcp-leader.md#to-read-quota-before-dispatch).
