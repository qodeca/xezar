...
The default team source is `qodeca/xezar-skills` at `main`. xezar reads it through a bare Git cache under `~/.cache/xez/skills/`, shared by every project on the machine; initial catalog loading runs in the background. In single-project mode the cache is inside the folder instead (`<project>/.local/xezar/cache/skills/`), so a clone fetches its own team skills. Without network access, it can list the cached copy or an empty team catalog. **Refresh** requests a new catalog fetch.
...
![Skills catalog and preview](../screenshots/0.18.0/skills-dark-1280.png)
