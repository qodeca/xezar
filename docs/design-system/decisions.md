...
| **Decision** | On `channel === 'dev'` only, a round badge sits on the brand tile's top-right corner: `absolute -top-1 -right-1 grid size-3.5 place-items-center rounded-full bg-danger text-[9px] leading-none font-semibold text-primary-foreground ring-2 ring-sidebar`, with a visible "D" (`aria-hidden`), `sr-only` text "Development build" and `title="Development build"`. **The fill is red because the owner asked for red**, knowing that red otherwise means broken: this is a documented exception for build identity, not a new status colour, and it does not decide D-01. The letter is `text-primary-foreground` – `#0d0d0d` in both themes, about 5.1:1 on `#ef4444` (computed from the hex values) – rather than a new token or raw black. Sizing stays on the scale (no hand-typed spacing pixel, no new allowlist row). The tile keeps `size-[26px]`, the badge overlays it, and the logo `<img>` keeps `alt=""`. `release`, `null` or an absent field (an older server) render the bare `<img>` exactly as before: no wrapper and no placeholder. |
| **Consequences** | The favicon, the phone top bar (it has no logo) and the MCP `health` tool stay unchanged (#439 follow-up). A second build-identity mark, or red for any other non-error marker, needs its own record. `cockpit.css` carries `.logo-wrap` and `.dev-badge` for mockups. The design review may resize the badge or letter (`size-3` with a smaller type size is the other candidate); a change of fill or letter colour supersedes this record. |
