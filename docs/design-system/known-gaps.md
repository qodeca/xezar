...
### G-43 Compare's pick confirm is irreversible but wears the ordinary CTA

- **Differs**: "Pick variant {A}" (`routes/compare-variants.tsx`) cancels the other variants and removes their worktrees and branches with no undo, and its dialog says so, yet `AlertDialogAction` keeps the default `contrast` look. G-10's rule tints every irreversible confirm; the Workflows and plan-review overwrites follow it since #453 B7.
- **Rule**: open – the action is a positive choice for the kept variant and a destructive one for the rest, so it needs a design verdict rather than a silent restyle.
- **Fix**: decide in the B7 design review whether the pick confirm takes `danger` (and whether "Keep comparing" stays the cancel); apply it in B8. Owner: B8 reconciliation of #453.

### G-44 Four documentation lines still describe the looks B7 replaced
