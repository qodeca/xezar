...
- **Fix**: not attempted here — it would touch either `packages/web/src/routes/tasks-overview.tsx` (shrink or wrap the header row) or a redesign of the header at narrow desktop widths, both out of scope for a docs task. Needs a decision: reproduce against the live cockpit at 800 px and file an issue if confirmed, or accept the header's 898 px minimum as the actual constraint and record it in `new-designs.md`/`patterns.md`.
- **Status (2026-09-18, docs wave PR 5)**: open, unowned. Flagged in the PR body for the leader.
