...
| Loading inside a surface that will hold a card | the same muted line, plus a numeric `min-h-*` reserving about one card, so the controls below do not jump when the answer lands. **Approximate by design**: a fixed reserve cannot match a sentence that wraps. Measured against `min-h-28` (#752, design review): −3.5 px in the default state, +13 to +46 px in the stale and never-checked states at 375 px, and −22 to +15 px across the density levers. Say "does not jump" of nothing; the claim is "under one text line in the default state". | "Checking…" with `min-h-28` (`routes/settings/skills-section.tsx`, `CatalogFields`) |
