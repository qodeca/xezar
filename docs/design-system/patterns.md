...
- Order: brand row (`xezar` + repo chip; on a development build the tile carries the red "D" badge, decisions.md D-08; in single-project mode a second line carries the neutral "Single project" badge, components.md AppShell, #600) → `New task` (`contrast` button with a `kbd` C) and `Add project` → nav (`NAV_ITEMS`, `<nav aria-label="Main">`, which fills the column and scrolls on a short window) → footer, one row (`sidebar-footer-controls`): Tools menu · version chip · Global settings · theme toggle. When an agent reports plan limits (#867), the footer opens with the plan-limits band above that row (`data-slot="agent-quota-band"`, components.md AgentQuotaChip) — a row of its own under the footer's one hairline, never a second elastic item in the controls (#702); with no applicable agent it renders no element and the footer is unchanged. There is no search launcher; ⌘K / Ctrl+K opens the palette from the keyboard (§7).
...
- The Tasks header carries the most controls (tabs, two count-gated actions, the 240 px search), so it takes `PageHeader`'s wrap and padding (`min-h-14 flex-wrap … py-2`) but keeps its own `gap-3` rather than `PageHeader`'s `gap-row`, and puts its actions and search in one right-aligned group (`ml-auto flex max-w-full flex-wrap justify-end gap-3`): in a narrow desktop window the group moves to a second row instead of scrolling the pane sideways, and a wide window still shows one 56 px row (#625).
...
- **The two-row variant** (`PageHeader` with `className="flex"`, #453 B7): Compare and Automations keep the header on a phone – the top bar there names only the area, and the task's own title or the sub-page title is what the page is about – and give the subtitle its own row with `order-last basis-full text-[13px] text-muted-foreground` instead of sitting it beside the title. The header is then about 96 px tall (Automations) and 94 px (Compare) against 70 px for the single-row Skills, Workflows and Inbox headers, measured at 1280 px in the light theme at the same density (B7 design review NB-6). It stays `min-h-14`, opaque, bordered, sticky and on the same `section` gutter, so only its height differs. Use it when the subtitle is a sentence rather than a few words; keep the single row otherwise. A header kept visible on a phone whose title would repeat the top bar word for word passes `titleClassName="sr-only md:not-sr-only"` – the Automations list does, its sub-pages do not (B7 design review NB-2).
- Variations that exist (two, both deliberate; #453 B7 retired G-01 by migrating the rest): the run header is `text-[15px]`, sticky only from `md`, and carries an editable title, so it is a different surface rather than a page header; and `/new` centres an `h1 text-lg` as a hero. GitHub takes the Git page's shape (opaque, `px-4 pt-stack md:px-section md:pt-group`, `h1 sr-only text-base font-semibold md:not-sr-only`), while Compare and Automations render the shared `PageHeader` with `className="flex"` so it stays visible on a phone.
...
| Loading inside a surface that will hold a card | a muted line in the surrounding page's text size (`text-[13px] text-soft-foreground` in Settings), left-aligned and unpadded, plus a numeric `min-h-*` reserving about one card, so the controls below do not jump when the answer lands. **Approximate by design**: a fixed reserve cannot match a sentence that wraps. Measured against `min-h-28` (#752, design review): −3.5 px in the default state, +13 to +46 px in the stale and never-checked states at 375 px, and −22 to +15 px across the density levers — measured on the up-to-date card; the longer cards run up to about +48 px at ultra and 375 px. Say "does not jump" of nothing; the claim is "under one text line in the default state". | "Checking…" with `min-h-28` (`routes/settings/skills-section.tsx`, `CatalogFields`) |
...
- Single-project mode (#600): the global area's scope chip (`data-slot="settings-scope-chip"`) and index title read "Workspace settings" instead of "Global settings", the global nav drops its "Stored in ~/.xezar" foot (the mode never opens it), and each section whose registry entry carries a `fileNote` opens with one file note, `data-slot="settings-file-note"`: a `FileIcon` at `size-3`, `text-[12px] text-soft-foreground`, "Saved in this project — " + the file as `<code class="rounded-sm bg-muted px-1 font-mono text-foreground">` + a full stop and an optional sentence. It sits at the top of the pane inside `mx-auto w-full max-w-2xl px-list pt-list md:px-group md:pt-group`, so it stays on a phone where the desktop header is hidden. It is text, never a link or a button. The file names live in one table beside `scope` in `registry.tsx`; a section that writes no xezar file (MCP API, MCP connection, Bookmarklets, Project setup, Agent config) shows none. Global mode renders neither change.
...
- State text beside a switch: a `<span class="text-[13px] text-muted-foreground">` inside the switch's `<label class="flex w-fit items-center gap-3">`, reading `On`, `Off` or `… (default)`. When the switch cannot decide the outcome on its own, a third state names that, and the reason moves to a hint paragraph below the label whose `id` the switch carries in `aria-describedby`, only while that state holds: Lock models reads "Off — still locked elsewhere" and its hint says where each outside lock is lifted (`routes/settings/agents-section.tsx`, `data-slot="agents-models-locked-elsewhere"`, #809).
- Plan limits (#867, Global settings → Agent accounts): a card block at the top of the pane (`id="limits"`, `data-slot="agent-quota-summary"`, `rounded-lg border border-border bg-card p-inset`) with one summary line per agent inside a polite `role="status"` WRAPPER — never on the `<ul>`, which would lose its list role — plus Refresh all and the "never acts" fine print; and a limits half in every Claude Code / Codex account row (`data-slot="account-limits"`, `role="group"` named "Plan limits of {name}, {agent}", after a dashed hairline): status sentence, one line per window (label · UsageBar · "{n}% used" · "resets {time}"; three lines below `md`), credits, the server's `warnings[]` (each a pending dot and its sentence, verbatim), and a meta line whose Refresh is `aria-disabled` (still focusable, the Button's "unavailable but focusable" state – never faded) with the reason on screen as its `aria-describedby`; Refresh is absent where a check cannot help (tool missing or too old, API-key login). Its detail half joins the row's own Show details panel (`routes/settings/account-limits.tsx`).
...
- The top bar titles itself from `activeNavItem(pathname)`. Its `data-slot="mobile-status"` slot carries the single-project mode badge in that mode (#600), so the mode is legible without opening the drawer, and the phone form of the plan-limits chip (`2/5 can work`, #867) when an agent applies; the title truncates first. The drawer carries no plan-limits band.
...
- Reconcile (invalidate runs, index, todos, health, worktrees, provider status, the MCP leader status, the plan limits) on every reconnect, on `visibilitychange` to visible and on bfcache `pageshow`. A closed stream reopens after 3 s.
- One WebSocket on `/api/v1/ws`, opened on the first `subscribeTopic` and closed when no topic is held. Three topics today, all only when `capabilities.localHandoff` is true: `health` and `agent-quota` (#867, the plan-limits answer the chip on every page reads), session-global, each subscribed once at the root (`useHealthSubscription`, `useAgentQuotaSubscription`); and `mcp-leader`, view-level, subscribed by Settings → MCP connection's leader control while it is on screen (`useMcpLeaderSubscription`). Remote mode opens no WebSocket.
...
- Backstops: the cross-project index polls every 15 s; ages tick with `useNow(30_000)`. A hosted cockpit re-reads the plan limits on the SSE `agent-quota` change hint and every 15 minutes while the tab is visible — the one `refetchInterval` here, because a hosted cockpit has no socket to be pushed through (#867 FR-11).
