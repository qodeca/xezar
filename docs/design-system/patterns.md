...
- The top bar titles itself from `activeNavItem(pathname)`. Its `data-slot="mobile-status"` slot carries the single-project mode badge in that mode (#600), so the mode is legible without opening the drawer; the title truncates first.
