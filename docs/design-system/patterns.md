...
- The Tasks header carries the most controls (tabs, two count-gated actions, the 240 px search), so it takes `PageHeader`'s wrapping spelling (`min-h-14 flex-wrap … py-2`) and puts its actions and search in one right-aligned group (`ml-auto flex max-w-full flex-wrap justify-end gap-3`): in a narrow desktop window the group moves to a second row instead of scrolling the pane sideways, and a wide window still shows one 56 px row (#625).
