...
- **Rules**: consumers import from `@/components/diff` only. Consumers set `--diff-sticky-top` so file headers park below the page header. Below `md` the view is forced to unified + wrap. The review panel and the compare view reach this same engine through `RunDiff` (below).
