...
describe('parseNewTaskParams', () => {
  const cases: Array<[name: string, search: string, expected: NewTaskParams]> = [
    ...
    [
      ...
      '?skill=xez-code-review&ref=https%3A%2F%2Fgithub.com%2Fo%2Fr%2Fpull%2F1&auto=1&key=s3cret',
      { skill: 'xez-code-review', ref: 'https://github.com/o/r/pull/1', auto: true, key: 's3cret', todo: '' },
    ],
    ['skill only (the "open the composer with this skill" bookmarklet)', '?skill=xez-fix', { ...empty, skill: 'xez-fix' }],
    ...
    ['whitespace around skill/ref is trimmed', '?skill=%20xez-fix%20&ref=%20x%20', { ...empty, skill: 'xez-fix', ref: 'x' }],
    ['unknown params are ignored', '?skill=xez-fix&nope=1', { ...empty, skill: 'xez-fix' }],
    // #374: the Inbox's Run adds `todo` so the composer can report the launch back.
    ['the inbox prefill link', '?skill=xez-fix&ref=fix+it&todo=t1', { ...empty, skill: 'xez-fix', ref: 'fix it', todo: 't1' }],
    ...
    ['no todo is the empty string, not undefined', '?skill=xez-fix&auto=1&key=k', { ...empty, skill: 'xez-fix', auto: true, key: 'k' }],
  ]
  ...
  it('takes a URLSearchParams too (what useSearchParams hands the route)', () => {
    expect(parseNewTaskParams(new URLSearchParams('skill=xez-fix&auto=1'))).toEqual({
      ...empty,
      skill: 'xez-fix',
      auto: true,
    })
...
describe('newTaskPrefillHref', () => {
  it('emits only the params it was given', () => {
    expect(newTaskPrefillHref({ skill: 'xez-fix', ref: 'fix it', todo: 't1' })).toBe(
      '/new?skill=xez-fix&ref=fix+it&todo=t1',
    )
  ...
  it('NEVER emits auto/key — not even alongside a todo id (#355: no blind launch)', () => {
    const params = new URLSearchParams(
      newTaskPrefillHref({ skill: 'xez-fix', ref: 'fix it', todo: 't1' }).split('?')[1],
    )
  ...
  it('round-trips through the parser (the link the Inbox builds is the one /new reads)', () => {
    const href = newTaskPrefillHref({ skill: 'xez-fix', ref: 'fix it\nnow', todo: 't1' })
