...
describe('C4 / G-10 irreversible confirms wear the danger button', () => {
  ...
  // patterns.md §7 asks for "There is no undo." wherever it is true; the plan review's overwrite is
  // as irreversible as the Workflows one and said so in neither word (B7 review NB-5).
  it('both irreversible overwrites say “There is no undo.”', () => {
    expect(code('routes/plan-review.tsx')).toContain('There is no undo.')
    expect(code('routes/workflows/workflows.tsx')).toContain('There is no undo.')
  })

  // Radix hands focus back only to its own trigger; the Compare confirm has none, so it needs the
  // shared hook the settings dialogs use (B7 review NB-3). The rendered proof is in
  // `routes/compare-variants.test.tsx`; this pins that the hook is the mechanism.
  it('Compare’s pick confirm returns focus through the shared hook', () => {
    const compare = code('routes/compare-variants.tsx')
    expect(compare).toMatch(/import \{ useReturnFocus \} from '@\/routes\/settings\/remove-project'/)
    expect(compare).toContain('const returnFocus = useReturnFocus(confirming !== null)')
    expect(compare).toContain('<AlertDialogContent onCloseAutoFocus={returnFocus}>')
  })
})
