...
let runId: string
/** Populated in `beforeAll` from the product's own `POST /api/v1/projects` response — see the
 *  comment above `apiRegisterProject` for why this file registers A and B through the real route
 *  instead of writing them into the shared registry file directly. */
let PROJECT_A: { id: string; name: string }
let PROJECT_B: { id: string; name: string }
...
/**
 * Register a scratch repo through the product's own door, `POST /api/v1/projects`, rather than
 * writing it into the shared registry file the way `MISSING` below and `project-groups.e2e.ts`
 * do. `project-context.ts` caches one `{store, manager, dataDir}` bundle per project id for the
 * life of the server process, and the ONLY call site that invalidates it is the project-removal
 * route (`server.ts`'s `DELETE /projects/:projectId` → `contexts.dispose(id)`). A project id
 * seeded directly into `config.json` and later deleted the same way (by rewriting the file) never
 * calls that route, so a second invocation against the SAME warm, reused server (the documented
 * local-iteration workflow, `docs/testing/agent-browser.md` § "boot once, reuse") finds the first
 * run's now-deleted `dataDir` still cached — the run this file creates against project A then
 * fails with `ENOENT` deep in the task engine. Registering AND removing through the real routes
 * makes this file re-runnable against a warm instance the same way a person clicking "Remove"
 * before re-adding a project would be.
 */
async function apiRegisterProject(root: string): Promise<{ id: string; name: string }> {
  ...
  if (response.status !== 200) {
    throw new Error(`xezar e2e: POST /api/v1/projects for ${root} answered ${response.status}`)
  }
  const body = (await response.json()) as { project: { id: string; name: string } }
  return body.project
}

/** The other half of `apiRegisterProject` — disposes the cached project context, so a later
 *  invocation against the same server never sees this run's `dataDir`. */
async function apiRemoveProject(projectId: string): Promise<void> {
  await fetch(`${baseUrl}/api/v1/projects/${projectId}`, { method: 'DELETE' })
}
...
beforeAll(async () => {
  ...
  // A and B go through the real registration route (see `apiRegisterProject`) so their teardown
  // can go through the real removal route too. `MISSING` never gets a run created against it and
  // never builds a project context, so seeding it straight into the registry file — the way
  // `project-groups.e2e.ts` seeds all of its projects — carries none of that risk.
  PROJECT_A = await apiRegisterProject(rootA)
  PROJECT_B = await apiRegisterProject(rootB)
  writeSharedProjects([
    ...readSharedProjects(),
...
afterAll(async () => {
  browser?.close()
  // Dispose A and B's cached project contexts through the real route BEFORE restoring the
  // registry file — see `apiRegisterProject`. `restoreHome()` alone would drop both ids from
  // `config.json` but leave the server's in-memory `dataDir` cache pointing at this run's
  // now-deleted worktree, which is exactly the staleness case E exists to catch.
  if (PROJECT_A) await apiRemoveProject(PROJECT_A.id)
  if (PROJECT_B) await apiRemoveProject(PROJECT_B.id)
  restoreHome?.()
