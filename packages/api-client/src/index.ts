/**
 * `@qodeca/xezar-api-client` — the one boundary between the xezar service and anything
 * that talks to it.
 *
 ...
 * (`packages/web`) and imported by the Node service (`packages/xezar`) without dragging either
 * runtime into the other — an invariant that used to be a comment on a hand-copied file and is
 * now structural.
 *
 * Contents:
 *   - `client.ts` — `createXezarClient<AppType>()`, the typed client over the versioned
 *     `/api/v1` surface. Types come from the server's own handlers; nothing is declared twice.
 *   - `@qodeca/xezar-contract` — re-exported wholesale, so a consumer needs one import for
...
export * from '@qodeca/xezar-contract'
