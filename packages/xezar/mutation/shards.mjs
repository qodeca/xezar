...
/**
 * Eight shards. Six was the original estimate (see the superseded reasoning this replaces,
 * below) — measured against real cost per #443's incident, it is no longer enough on its own.
 *
 ...
 *
 * THE FIX HAS TWO PARTS, because either alone measured short: eight shards instead of six, and
 * `loadWeights` below, which packs by MEASURED per-file cost when `docs/testing/mcp-mutation-shard-weights.json`
 * has an entry for a file, falling back to byte size only for a file the snapshot has never seen
 * (a genuinely new module). Weight-balancing six shards on the incident's own numbers already cut
 * the heaviest shard from 359 164 to 206 620 (a 42% cut) with NO added job; going to eight on top
 * of that leaves headroom for the snapshot going stale as the suite keeps growing — the single
 * heaviest file, `event-controller.ts` at 178 734, is what a shard count above eight stops helping
 * with, since one file's own cost floors whatever shard holds it. More shards would shorten each
 * job and add one `npm ci` apiece; fewer would eat the headroom this incident showed is required.
 */
