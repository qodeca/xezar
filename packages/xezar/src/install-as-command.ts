 ...
 *  `xezar`/`xez` command on PATH pointing at THIS working tree —
 *  the local-dev equivalent of `npm install -g @qodeca/xezar`, with no publish. This module
...
export const PACKAGE_NAME = '@qodeca/xezar';

/** Every bin the published package installs. Kept in lockstep with the `bin` map in
 *  `packages/xezar/package.json`; `test/e2e/package-exports.test.ts` pins that manifest to
 *  exactly these two names, so a local install exposes what a published install does. */
export const BIN_NAMES = ['xezar', 'xez'] as const;
...
/** Expected shim paths for every bin under an `npm prefix -g` value.
