 ...
 *
 * Driven through `XEZ_DRY_RUN=1`, so no `gh` is touched: the assertion is that the routes CALL
...
describe('a reference xezar changes itself is forgotten, not waited out', () => {
  let repoRoot: string;
  let store: RunStore;
  let app: Hono;
