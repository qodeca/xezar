...
export function createApp(deps: ServerDeps) {
    ...
    .post('/workspace/agent-profiles', localHandoffRoute, jsonZodValidator(() => createAgentProfileInputSchema),
      ...
      try {
        await mergeWriteAgentAccounts((store) => {
          // An identity-shaped label is still stored for the person's pane, but never used as the
          // public id/handle: slugging an email only changes its punctuation, it does not redact it
          // (#677 B6, QA F1 on #764). Existing stored ids are read untouched; this applies only at
          // allocation time for a new row.
          const idSource = label?.includes('@') ? `account-${randomUUID().slice(0, 8)}` : (label ?? configDir);
  ...
  const runsRoutes = new Hono<ProjectApiEnv>()
    ...
    .post('/runs/:id/pr', optionalJsonZodValidator(runVersionGuardInputSchema, { absent: ({}) }), ui.route('run.pr.create', { resource: { kind: 'run', param: 'id' } }), async (c) => {
      ...
      if (!run) return store.getRun(id) ? c.json({ error: FOREIGN_WORKTREE }, 409) : c.json({ error: 'not found' }, 404);
