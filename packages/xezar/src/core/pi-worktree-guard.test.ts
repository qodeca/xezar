...
describe('pi read-only lock adapter (#863)', () => {
  type GuardApi = Parameters<typeof extension>[0];
  type Handler = Parameters<GuardApi['on']>[1];
  ...
  const inPlaceRun = (entries: readonly string[], command: string) =>
    load({ 'xezar-bash-allowlist': JSON.stringify(entries) })(bash(command), { cwd: tmpdir() });

  it.each(READ_ONLY_LOCK_FIXTURES)('matches the shared fixture: $name', ({ command, entries, rule }) => {
    const result = inPlaceRun(entries, command);
    if (rule === undefined) expect(result).toBeUndefined();
    else {
      expect(result).toMatchObject(BLOCK);
      expect(result?.reason).toContain(`Rule ${rule}`);
    }
  });

  it('keeps pi-specific flag parsing and denial transport in the adapter', () => {
    for (const flag of ['not-json', '[1]', '{}', '"git"', true]) {
      const result = load({ 'xezar-bash-allowlist': flag })(bash('git diff'), { cwd: tmpdir() });
      expect(result).toMatchObject(BLOCK);
      expect(result?.reason).toContain('flag is not null or a JSON list of strings');
    }
  });

  it('reads null as no allowlist and refuses a missing flag', () => {
    const f = fixture();
    const roots = { 'xezar-worktree-root': f.worktree, 'xezar-primary-root': f.primary };
    expect(load({ ...roots, 'xezar-bash-allowlist': 'null' })(bash('git status'), { cwd: f.worktree })).toBeUndefined();
    const absent = load(roots);
    expect(absent(bash('git status'), { cwd: f.worktree })?.reason).toContain('flag is missing');
    expect(absent(write('notes.md'), { cwd: f.worktree })).toBeUndefined();
  });

  it('keeps the worktree guard after the shared policy allows a command', () => {
    const f = fixture();
    const handler = load({
      'xezar-worktree-root': f.worktree,
      'xezar-primary-root': f.primary,
      'xezar-bash-allowlist': JSON.stringify(['git']),
    });
    expect(handler(bash('git status'), { cwd: f.worktree })).toBeUndefined();
    expect(handler(bash(`git -C ${f.primary} status`), { cwd: f.worktree })).toMatchObject(BLOCK);
  });

  it('fails closed when only one worktree flag is present', () => {
    const f = fixture();
