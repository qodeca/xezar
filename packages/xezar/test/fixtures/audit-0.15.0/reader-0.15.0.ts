// FROZEN COPY — the audit reader exactly as xezar 0.15.0 shipped it. Never edit this file to make a
// test pass: it stands in for the released version a user may downgrade to (#306, spec
// `docs/features/mcp-server/audit-trail-origins-2026-09-17.md` § 8 and § 11 "Old-reader contract").
//
// Sources, byte-for-byte below the marker lines:
//   - the schema: `git show v0.15.0:packages/contract/src/mcp-audit.ts` (blob a602ebc6)
//   - the loop:   `AuditTrail.read` of `git show v0.15.0:packages/xezar/src/mcp/audit-trail.ts`
//                 (blob 52da3aa4), with only `this.scope.projectId` / the path lifted into
//                 parameters so it runs without the 0.15.0 class around it.
// `audit-upgrade.test.ts` pins the schema half to the tag when the tag is available.

// ---- v0.15.0:packages/contract/src/mcp-audit.ts ----
import { z } from 'zod';
...
// ---- end of v0.15.0:packages/contract/src/mcp-audit.ts ----

// ---- AuditTrail.read, v0.15.0:packages/xezar/src/mcp/audit-trail.ts:175-203 ----
export const AUDIT_TRAIL_FILE_0_15_0 = 'mcp-audit.ndjson';
...
export function readAudit0150(raw: string, projectId: string): AuditReadResult {
