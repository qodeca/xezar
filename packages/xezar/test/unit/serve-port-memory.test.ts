...
async function bootServe(
  ...
  const child = spawn(
    process.execPath,
    ['--import', tsxLoader, entry, 'serve', '--no-open', '--repo', repo, ...args],
    {
      cwd: repo,
      env: {
        ...process.env,
  ...
  try {
    ...
    // The port is remembered right after the bind, which is a merge-write away — give it the
...
/** A port that is free right now, plus `span - 1` usable ports after it. */
async function freePort(span = 1): Promise<number> {
  for (;;) {
