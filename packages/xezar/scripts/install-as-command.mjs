...
const PACKAGE_NAME = planner?.PACKAGE_NAME ?? '@qodeca/xezar';
const steps = planner
...
if (mode === 'uninstall') {
  console.log('install-as-command: global xezar / xez removed.');
  process.exit(0);
}
...
if (planner && prefix) {
  ...
  console.log('  Then, from any repo:  xezar --help');
} else {
  console.log('\ninstall-as-command: done. Ensure your npm global bin dir is on PATH, then run `xezar --help`.');
}
